...
// FNV-1a 64-bit constants used to build a deterministic signature of generation
// inputs. That signature drives stage-cache invalidation in incremental mode.
...
template <typename T> void HashScalar(const T &value, uint64_t &hash) {
...
[[nodiscard]] Core::Bounds
ConstraintsBounds(const Core::WorldConstraintField &constraints) {
...
[[nodiscard]] bool BoundsMatch(const Core::Bounds &a, const Core::Bounds &b,
                               double tolerance) {
...
void AssertTextureMatchesConstraints(
  ...
  assert(texture_space.resolution() ==
         std::max(constraints.width, constraints.height));
#else
  (void)texture_space;
  (void)constraints;
#endif
...
[[nodiscard]] bool PointInPolygon(const Core::Vec2 &point,
...
// Returns true if the point falls inside the influence radius of any input
// axiom. A small minimum radius keeps tiny axioms from becoming impossible to
// hit.
[[nodiscard]] bool
PointInsideAnyAxiom(const Core::Vec2 &point,
...
// Splits roads into contiguous segments that remain inside axiom influence
// regions. Segments crossing outside influence are cut and emitted as separate
// roads.
...
[[nodiscard]] Core::Vec2
PolygonCentroid(const std::vector<Core::Vec2> &polygon) {
  if (polygon.empty()) {
    return {};
  }
...
// Treats a polygon as "touching" axiom influence if its centroid falls within
// influence. This is intentionally coarse and fast; we do not perform full
// polygon overlap tests here.
[[nodiscard]] bool
PolygonTouchesAnyAxiom(const std::vector<Core::Vec2> &polygon,
                       const std::vector<CityGenerator::AxiomInput> &axioms) {
  if (polygon.empty()) {
    return false;
  }
...
// Retains districts whose centroid-based footprint intersects any axiom
// influence zone.
...
// If boundary data exists we classify by polygon centroid, else by lot
// centroid.
[[nodiscard]] std::vector<Core::LotToken>
ClipLotsToAxiomInfluence(const std::vector<Core::LotToken> &lots,
                         const std::vector<CityGenerator::AxiomInput> &axioms) {
  if (axioms.empty()) {
    return lots;
  }
  ...
  for (const auto &lot : lots) {
    const bool inside = !lot.boundary.empty()
...
// Building-level influence clip: keep only sites whose placement point remains
// in-bounds.
...
[[nodiscard]] std::vector<Core::Vec2>
BuildCityBoundary(const std::vector<CityGenerator::AxiomInput> &axioms) {
  ...
  for (const auto &axiom : axioms) {
    radii.push_back(std::max(
        20.0, axiom.radius *
                  std::clamp(axiom.ring_schema.outskirts_ratio, 0.2, 1.5)));
  }
  ...
  for (const auto &axiom : axioms) {
    const double r = std::max(
        20.0,
        axiom.radius * std::clamp(axiom.ring_schema.outskirts_ratio, 0.2, 1.5));
    for (int i = 0; i < kSamples; ++i) {
      const double angle =
          (2.0 * M_PI * static_cast<double>(i)) / static_cast<double>(kSamples);
      cloud.emplace_back(axiom.position.x + std::cos(angle) * r,
  ...
  const bool closed = std::abs(cloud.front().x() - cloud.back().x()) < 1e-9 &&
...
[[nodiscard]] bool
TerrainConfigEqual(const TerrainConstraintGenerator::Config &a,
...
[[nodiscard]] bool
ValidatorConfigEqual(const PlanValidatorGenerator::Config &a,
...
// Full configuration equivalence used to decide whether cached stage outputs
// remain valid.
[[nodiscard]] bool ConfigEquivalent(const CityGenerator::Config &a,
                                    const CityGenerator::Config &b) {
  return a.width == b.width && a.height == b.height &&
         a.cell_size == b.cell_size && a.seed == b.seed &&
         a.num_seeds == b.num_seeds && a.max_districts == b.max_districts &&
         a.max_lots == b.max_lots && a.max_buildings == b.max_buildings &&
...
CityGenerator::generate(const std::vector<AxiomInput> &axioms) {
  return generate(axioms, Config{});
}

// Convenience overload: allows explicit config but no editor global-state
// integration.
CityGenerator::CityOutput
CityGenerator::generate(const std::vector<AxiomInput> &axioms,
                        const Config &config) {
...
CityGenerator::CityOutput CityGenerator::GenerateWithContext(
    const std::vector<AxiomInput> &axioms, const Config &config,
    GenerationContext *context, Core::Editor::GlobalState *global_state) {
  StageOptions options{};
  options.stages_to_run = FullStageMask();
  options.use_cache = false;
...
CityGenerator::CityOutput CityGenerator::RegenerateIncremental(
    const std::vector<AxiomInput> &axioms, const Config &config,
    StageMask dirty_stages, Core::Editor::GlobalState *global_state,
...
CityGenerator::ValidationResult
CityGenerator::ValidateAndClampConfig(const Config &config) {
  ValidationResult result{};
  result.clamped_config = config;

  auto clamp_int = [&result](int &value, int min_v, int max_v,
                             const char *label) {
    const int original = value;
    value = std::clamp(value, min_v, max_v);
    if (value != original) {
      result.warnings.emplace_back(std::string("Clamped ") + label + " to " +
                                   std::to_string(value));
    }
  };
  // Integer variant of the above helper.
  auto clamp_double = [&result](double &value, double min_v, double max_v,
                                const char *label) {
  ...
  // Range checks for basic world parameters. These are primarily to prevent
  // pathological memory usage and ensure stable generation.
  ...
  if (result.clamped_config.max_texture_resolution <= 64) {
    result.warnings.emplace_back(
        "max_texture_resolution too low; raised to 64");
    result.clamped_config.max_texture_resolution = 64;
  }
  // The cell size and max texture resolution are interdependent based on world
  // size, so we perform a preliminary clamping pass before validating their
  // combined constraints.
  const double world_max = static_cast<double>(
      std::max(result.clamped_config.width, result.clamped_config.height));
  int implied_resolution = 1;
  if (result.clamped_config.cell_size > 0.0) {
    implied_resolution = static_cast<int>(
        std::ceil(world_max / result.clamped_config.cell_size));
  }
  // If the implied resolution from the cell size exceeds the max texture
  // resolution, we need to increase the cell size to avoid generating unusably
  // large textures.
  if (implied_resolution > result.clamped_config.max_texture_resolution) {
    const double adjusted =
        world_max /
        static_cast<double>(result.clamped_config.max_texture_resolution);
  ...
  // After clamping the basic world parameters, we can derive conservative upper
  // bounds for district/lot/building counts to prevent pathological cases on
  // small maps and unbounded memory growth on large maps.
  DeriveConstraintsFromWorldSize(result.clamped_config, result);

  // Seed 0 is technically valid but can lead to silent failures in some
  // downstream algorithms that use zero as a sentinel value, so we proactively
  // clamp it to 1 and warn the user.
  if (result.clamped_config.seed == 0u) {
    result.warnings.emplace_back(
        "seed was 0; replaced with 1 for deterministic runs");
    result.clamped_config.seed = 1u;
  }
  // We allow users to set very high iteration counts for stress-testing and
  // profiling, but we clamp the minimum to 1 to prevent no-op runs that can be
  ...
  // Trace step sizes are clamped to reasonable ranges to prevent extreme values
  // that can cause instability or excessively long generation times. We also
  // enforce that the max step size is not smaller than the min step size,
  // swapping them if necessary and issuing a warning.

  result.clamped_config
      .min_trace_step_size = // We allow very small step sizes for high-detail
                             // tracing, but we clamp the minimum to 0.5 to
                             // prevent excessively long generation times and
                             // potential numerical instability.

      std::clamp(result.clamped_config.min_trace_step_size, 0.5,
                 50.0); // We allow very large step sizes for fast, low-detail
                        // tracing, but we clamp the maximum to 100 to prevent
                        // completely skipping over important features and
                        // causing generation failures.

  result.clamped_config
      .max_trace_step_size = // We allow very large step sizes for fast,
                             // low-detail tracing, but we clamp the maximum to
                             // 100 to prevent completely skipping over
                             // important features and causing generation
                             // failures.

      std::clamp(result.clamped_config.max_trace_step_size, 0.5,
                 100.0); // we enforce that the max step size is not smaller
                         // than the min step size, swapping them if necessary
                         // and issuing a warning.

  // If the max step size is smaller than the min step size, swap them and issue
  // a warning. This is a common user error that can lead to confusing behavior,
  // so we handle it gracefully rather than treating it as a hard error.
  if (result.clamped_config.max_trace_step_size <
      result.clamped_config.min_trace_step_size) {

    std::swap(
        result.clamped_config.max_trace_step_size,
        result.clamped_config
            .min_trace_step_size); // After swapping, we also clamp them again
                                   // to ensure they remain within the valid
                                   // ranges defined above, in case the original
                                   // values were outside those ranges.
    result.clamped_config.min_trace_step_size = std::clamp(
        result.clamped_config.min_trace_step_size, 0.5,
        50.0); // After swapping, we also clamp them again to ensure they remain
               // within the valid ranges defined above, in case the original
               // values were outside those ranges.
    result.clamped_config.max_trace_step_size = std::clamp(
        result.clamped_config.max_trace_step_size, 0.5,
        100.0); // After swapping, we also clamp them again to ensure they
                // remain within the valid ranges defined above, in case the
                // original values were outside those ranges.

    result
        .warnings.emplace_back("trace step bounds were inverted and have been "
                               "swapped"); // After swapping and clamping, we
...
// Performs semantic validation of axiom parameters against world bounds and
// type-specific limits. Returns true if all axioms are valid, otherwise
// populates the errors vector with validation messages.
bool CityGenerator::ValidateAxioms(const std::vector<AxiomInput> &axioms,
                                   const Config &config,
  ...
  for (size_t i = 0; i < axioms.size(); ++i) {
    ...
    const bool in_bounds =
        axiom.position.x >= 0.0 && axiom.position.y >= 0.0 &&
    ...
    const bool has_warp_payload =
        !axiom.warp_lattice.vertices.empty() || axiom.warp_lattice.rows > 0 ||
        axiom.warp_lattice.cols > 0 || axiom.warp_lattice.topology_type != 0;
    if (has_warp_payload) {
      if (axiom.warp_lattice.topology_type < 0 ||
          axiom.warp_lattice.topology_type > 3) {
        local_errors.push_back(prefix +
                               "warp_lattice.topology_type out of range");
      }
      if (axiom.warp_lattice.zone_inner_uv <= 0.0f ||
          axiom.warp_lattice.zone_middle_uv <
              axiom.warp_lattice.zone_inner_uv ||
          axiom.warp_lattice.zone_outer_uv <
              axiom.warp_lattice.zone_middle_uv ||
      ...
      for (size_t v = 0; v < axiom.warp_lattice.vertices.size(); ++v) {
        ...
        if (!std::isfinite(point.x) || !std::isfinite(point.y)) {
          local_errors.push_back(prefix + "warp_lattice vertex[" +
      ...
      if (axiom.warp_lattice.topology_type == 0) {
        if (axiom.warp_lattice.rows <= 0 || axiom.warp_lattice.cols <= 0) {
          local_errors.push_back(prefix +
                                 "bezier warp_lattice requires rows/cols > 0");
        } else if (axiom.warp_lattice.vertices.size() !=
                   static_cast<size_t>(axiom.warp_lattice.rows *
                                       axiom.warp_lattice.cols)) {
          local_errors.push_back(prefix +
                                 "bezier warp_lattice vertex count mismatch");
        }
      } else if (axiom.warp_lattice.topology_type == 1 &&
                 axiom.warp_lattice.vertices.size() < 3) {
        local_errors.push_back(prefix +
                               "polygon warp_lattice requires >= 3 vertices");
      } else if (axiom.warp_lattice.topology_type == 2 &&
                 axiom.warp_lattice.vertices.size() < 2) {
        local_errors.push_back(prefix +
                               "radial warp_lattice requires >= 2 vertices");
      } else if (axiom.warp_lattice.topology_type == 3 &&
                 axiom.warp_lattice.vertices.size() < 2) {
        local_errors.push_back(prefix +
                               "linear warp_lattice requires >= 2 vertices");
      }
    }
    ...
    for (size_t r = 0; r < axiom.radial_ring_knob_weights.size(); ++r) {
      for (size_t k = 0; k < axiom.radial_ring_knob_weights[r].size(); ++k) {
        ...
        if (!std::isfinite(knob) || knob < 0.1f || knob > 3.0f) {
          local_errors.push_back(prefix + "radial_ring_knob_weights[" +
    ...
    }
    {
      ...
      if ((axiom.terminal_features.bits & ~allowed_bits) != 0ull) {
        local_errors.push_back(
            prefix +
            "terminal_features contains flags not supported by axiom type");
      }
    }
    ...
    {
      const auto feature_conflicts =
          validateFeatureConflicts(axiom.terminal_features);
    ...
    case AxiomInput::Type::Suburban:
      ...
      if (axiom.suburban_loop_strength < 0.0f ||
          axiom.suburban_loop_strength > 1.0f) {
        local_errors.push_back(prefix +
                               "suburban_loop_strength must be in [0,1]");
      }
      break;
    case AxiomInput::Type::Stem:
      // Validate stem_branch_angle is positive and finite.
      if (!(axiom.stem_branch_angle > 0.0f) ||
          !std::isfinite(axiom.stem_branch_angle)) {
        local_errors.push_back(prefix +
                               "stem_branch_angle must be finite and > 0");
      }
      break;
    case AxiomInput::Type::Superblock:
      // Validate superblock_block_size is positive and finite.
      if (!(axiom.superblock_block_size > 0.0f) ||
          !std::isfinite(axiom.superblock_block_size)) {
        local_errors.push_back(prefix +
                               "superblock_block_size must be finite and > 0");
      }
      break;
    ...
    case AxiomInput::Type::Hexagonal:
      // Hex fields collapse when the envelope is too small relative to sample
      // spacing.
      if (axiom.radius < config.cell_size * 2.0) {
        local_errors.push_back(prefix +
                               "hexagonal radius must be >= 2 * cell_size");
      }
      break;
    case AxiomInput::Type::Linear:
      // Linear fields need a minimum reach so streamline tracing has
      // directionality.
    ...
    case AxiomInput::Type::GridCorrective:
      // Corrective overlays still require a non-trivial footprint to be
      // meaningful.
      if (axiom.radius < config.cell_size) {
        local_errors.push_back(prefix +
                               "grid-corrective radius must be >= cell_size");
      }
      break;
...
// perhaps consider an underterminstic but fast hash for the axioms if
// performance of this becomes an issue, but for now we will prioritize
// correctness and simplicity with a deterministic FNV-1a implementation that
// treats floating-point values carefully to ensure consistent signatures across
// platforms and runs.
...
// through each axiom and hashing its fields in a consistent order. We use the
// FNV-1a algorithm for hashing, which is simple and has good distribution
// properties for small inputs. We also take care to normalize floating-point
// values to ensure that equivalent values produce the same hash, such as
// treating +0.0 and -0.0 as equal. The resulting hash value can be used as part
// of the cache signature to determine if cached stage outputs remain valid when
// axioms change.
uint64_t CityGenerator::HashAxioms(const std::vector<AxiomInput> &axioms) {
  ...
  // We iterate through each axiom and hash its fields in a consistent order.
  // The field order is explicitly defined here to ensure that the same set of
  // axioms always produces the same hash, regardless of any internal memory
  // layout or padding differences. We also normalize floating-point values to
  // ensure that equivalent values produce the same hash, such as treating +0.0
  // and -0.0 as equal.
  for (const auto &axiom : axioms) {
    ...
    HashScalar(static_cast<uint8_t>(axiom.ring_schema.strict_core ? 1u : 0u),
               hash);
    HashScalar(static_cast<uint8_t>(axiom.lock_generated_roads ? 1u : 0u),
               hash);
...
// This prevents pathological counts on small maps and unbounded memory growth
// on large maps.
void CityGenerator::DeriveConstraintsFromWorldSize(Config &config,
  ...
  const uint32_t derived_district_cap = static_cast<uint32_t>(std::clamp(
      static_cast<long long>(std::llround(area_km2 / 0.25)), 16ll, 4096ll));
  // - districts: ~1 per 0.25 km^2
  const uint32_t derived_lot_cap = static_cast<uint32_t>(
      std::clamp(static_cast<long long>(std::llround(area_km2 * 2500.0)), 500ll,
                 500000ll));
  // - lots: ~2500 per km^2
  const uint32_t derived_building_cap = static_cast<uint32_t>(std::clamp(
      static_cast<long long>(derived_lot_cap) * 2ll, 1000ll, 1000000ll));
  // - buildings: up to 2x lots
  if (config.max_districts > derived_district_cap) {
    result.warnings.emplace_back(
        "max_districts exceeded derived cap; clamped to " +
        std::to_string(derived_district_cap));
    config.max_districts = derived_district_cap;
  }
  if (config.max_lots > derived_lot_cap) {
    result.warnings.emplace_back("max_lots exceeded derived cap; clamped to " +
                                 std::to_string(derived_lot_cap));
    config.max_lots = derived_lot_cap;
  }
  if (config.max_buildings > derived_building_cap) {
    result.warnings.emplace_back(
        "max_buildings exceeded derived cap; clamped to " +
        std::to_string(derived_building_cap));
...
void CityGenerator::InvalidateCacheIfInputChanged(const Config &config,
                                                  uint64_t axioms_hash) {
  if (!cache_.has_signature || !ConfigEquivalent(cache_.config, config) ||
...
CityGenerator::CityOutput
CityGenerator::GenerateStages(const std::vector<AxiomInput> &axioms,
                              const Config &config, const StageOptions &options,
                              Core::Editor::GlobalState *global_state,
                              GenerationContext *context) {
  CityOutput output{};
  ...
  if (global_state == nullptr ||
      global_state->config.feature_axiom_overlap_resolution) {
  ...
  // Materialize output from cache snapshot (the cache is the single source of
  // truth between runs).
  const auto copy_cache_to_output = [&]() {
    ...
    // Include 3D foundation data - intersection templates surfaced as
    // first-class output
  ...
  // In "exact stage" mode (no cascade), hide non-requested artifacts from
  // returned output. Cache is still retained; this only shapes what the caller
  // receives.
  const auto trim_output_for_partial_stages = [&]() {
    ...
    if (!IsStageDirty(dirty, GenerationStage::Roads)) {
      ...
      output.intersection_templates
          .clear(); // Clear 3D intersection data when roads not requested
    }
  ...
  // Generates topography/environment masks plus a coarse site profile used by
  // later stages.
  if (config_.enable_world_constraints) {
    ...
    if (should_run_stage(GenerationStage::Terrain)) {
      // Initialize the TerrainConstraintGenerator to generate terrain
      // constraints.
      ...
      terrain_input.cell_size =
          config_.cell_size;             // Size of each cell in the grid.
      terrain_input.seed = config_.seed; // Seed for random generation.

      // Generate terrain constraints and site profile based on the input
      // configuration.
    ...
    // If world constraints are disabled, initialize empty constraints and site
    // profile.
    cache_.world_constraints =
        WorldConstraintField{};          // Empty world constraints.
  ...
  // Keep texture space aligned with current world bounds/constraints before
  // writing downstream layers.
  const WorldConstraintField *constraints =
      cache_.world_constraints.isValid() ? &cache_.world_constraints : nullptr;
  initializeTextureSpaceIfNeeded(global_state, constraints);
  const Core::Data::TextureSpace *texture_space =
      (global_state != nullptr && global_state->HasTextureSpace())
          ? &global_state->TextureSpaceRef()
          : nullptr;
  ...
  if (should_run_stage(GenerationStage::Roads)) {
    const SiteProfile *site_profile =
        constraints != nullptr ? &cache_.site_profile : nullptr;
    std::vector<Vec2> seeds = generateSeeds(
        constraints, texture_space, context,
    ...
    cache_.roads =
        traceRoads(cache_.tensor_field, seeds, constraints, site_profile,
                   texture_space, context, global_state, &resolved_axioms);

    // Capture intersection templates from road generation (surface as
    // first-class output)
    ...
    if (global_state == nullptr ||
        global_state->config.feature_major_connector_graph) {
      MajorConnectorGraph graph_builder;
      const MajorConnectorGraphOutput connector_graph =
          graph_builder.Build(resolved_axioms);
  ...
  if (should_run_stage(GenerationStage::Districts)) {
    ...
    if (options.constrain_roads_to_axiom_bounds) {
      cache_.districts =
          ClipDistrictsToAxiomInfluence(cache_.districts, resolved_axioms);
    }
  ...
  if (should_run_stage(GenerationStage::Lots)) {
    const SiteProfile *site_profile =
        constraints != nullptr ? &cache_.site_profile : nullptr;
    cache_.lots = generateLots(cache_.roads, cache_.districts, cache_.blocks,
                               site_profile, context);
    if (options.constrain_roads_to_axiom_bounds) {
  ...
  // Stage 6: Buildings.
  // Places building sites on lots, then performs texture-driven feasibility
  // filtering.
  if (should_run_stage(GenerationStage::Buildings)) {
    ...
    if (texture_space != nullptr) {
      cache_.buildings = filterBuildingsByTexture(
          cache_.buildings, cache_.lots, cache_.districts, *texture_space);
    }
    if (options.constrain_roads_to_axiom_bounds) {
      cache_.buildings =
  ...
  if (global_state == nullptr ||
      global_state->config.feature_city_boundary_hull) {
  ...
  if (constraints != nullptr) {
    if (should_run_stage(GenerationStage::Validation)) {
      ...
      auto validation_output =
          validator.validate(validator_input, config_.validator);
...
CityGenerator::generateTensorField(const std::vector<AxiomInput> &axioms,
                                   GenerationContext *context) {
  // Configure the dimensions and cell size of the tensor field.
  TensorFieldGenerator::Config field_config;
  field_config.width = std::max(
      1, static_cast<int>(config_.width /
                          config_.cell_size)); // Ensure width is at least 1.
  field_config.height = std::max(
      1, static_cast<int>(config_.height /
                          config_.cell_size)); // Ensure height is at least 1.
  field_config.cell_size =
      config_.cell_size; // Set the size of each cell in the field.
  ...
  for (const auto &axiom : axioms) {
    ...
    if ((axiom.type == AxiomInput::Type::Linear ||
         axiom.type == AxiomInput::Type::Stem) &&
    ...
    switch (axiom.type) {
    case AxiomInput::Type::Organic:
      field.addOrganicField(axiom.position, axiom.radius, theta_to_use,
                            axiom.organic_curviness, axiom.decay);
      break;
    case AxiomInput::Type::Grid:
      field.addGridField(axiom.position, axiom.radius, theta_to_use,
                         axiom.decay);
      break;
    case AxiomInput::Type::Radial:
      field.addRadialField(axiom.position, axiom.radius, axiom.radial_spokes,
                           axiom.radial_ring_rotation,
                           axiom.radial_ring_knob_weights, axiom.decay);
      break;
    case AxiomInput::Type::Hexagonal:
      field.addHexagonalField(axiom.position, axiom.radius, axiom.theta,
                              axiom.decay);
      break;
    case AxiomInput::Type::Stem:
      field.addStemField(axiom.position, axiom.radius, theta_to_use,
                         axiom.stem_branch_angle, axiom.decay);
      break;
    case AxiomInput::Type::LooseGrid:
      field.addLooseGridField(axiom.position, axiom.radius, axiom.theta,
                              axiom.loose_grid_jitter, axiom.decay);
      break;
    case AxiomInput::Type::Suburban:
      field.addSuburbanField(axiom.position, axiom.radius,
                             axiom.suburban_loop_strength, axiom.decay);
      break;
    case AxiomInput::Type::Superblock:
      field.addSuperblockField(axiom.position, axiom.radius, axiom.theta,
                               axiom.superblock_block_size, axiom.decay);
      break;
    case AxiomInput::Type::Linear:
      field.addLinearField(axiom.position, axiom.radius, theta_to_use,
                           axiom.decay);
      break;
    case AxiomInput::Type::GridCorrective:
      field.addGridCorrective(axiom.position, axiom.radius, axiom.theta,
                              axiom.decay);
      break;
    case AxiomInput::Type::COUNT:
    ...
    FeaturePlan feature_plan =
        buildFeaturePlan(axiom.type, tensor_params, axiom.terminal_features,
    ...
    if (!axiom.terminal_features.empty()) {
      std::cerr << "[TensorFeaturePlan] axiom_id=" << axiom.id
                << " type=" << static_cast<int>(axiom.type) << " features={";
      const auto allowed_features = featuresForAxiomType(axiom.type);
      bool first = true;
      ...
      std::cerr << "} radius=" << axiom.radius << " theta=" << axiom.theta
                << " ring_rot=" << axiom.radial_ring_rotation << std::endl;
    }
#endif
  ...
  // Perform a final solve pass to bake all influences into the tensor field
  // grid.
  if (!ShouldAbort(context)) {
    field.generateField();
  }
...
// 3) fallback to deterministic center/axiom-position seeds if no valid sample
// was found
std::vector<Vec2>
CityGenerator::generateSeeds(const WorldConstraintField *constraints,
                             const Core::Data::TextureSpace *texture_space,
                             GenerationContext *context,
  ...
  if (seeds.empty()) {
    if (seed_axiom_bounds != nullptr && !seed_axiom_bounds->empty()) {
      for (const auto &axiom : *seed_axiom_bounds) {
        seeds.push_back(Vec2(std::clamp(axiom.position.x, 0.0,
                                        static_cast<double>(config_.width)),
                             std::clamp(axiom.position.y, 0.0,
                                        static_cast<double>(config_.height))));
      }
    }
    if (seeds.empty()) {
      seeds.push_back(Vec2(static_cast<double>(config_.width) * 0.5,
...
// Constraint/profile data can tighten tracing policy for environmentally
// sensitive modes. Axioms are scanned for alignment-demanding terminal features
// to configure tensor alignment.
fva::Container<Road> CityGenerator::traceRoads(
    const TensorFieldGenerator &field, const std::vector<Vec2> &seeds,
    const WorldConstraintField *constraints, const SiteProfile *profile,
    const Core::Data::TextureSpace *texture_space, GenerationContext *context,
    Core::Editor::GlobalState *global_state,
  ...
  road_cfg.tracing.separation_cell_size = std::max(
      road_cfg.tracing.min_separation, road_cfg.tracing.step_size * 2.0);
  ...
  if (axioms != nullptr && !axioms->empty()) {
    ...
    for (const auto &axiom : *axioms) {
      ...
      if (axiom.terminal_features.has(
              TerminalFeature::Grid_AxisAlignmentLock) ||
          axiom.terminal_features.has(
              TerminalFeature::Hex_HoneycombStrictness) ||
          axiom.terminal_features.has(
      ...
      if (axiom.terminal_features.has(
          ...
          axiom.terminal_features.has(
              TerminalFeature::GridCorrective_AbsoluteOverride)) {
    ...
    if (has_strong_alignment || has_grid_corrective) {
      ...
      road_cfg.tracing.tensor_alignment_strength =
          has_grid_corrective ? 0.45 : 0.35;
      road_cfg.tracing.tensor_alignment_tolerance_degrees =
          has_grid_corrective ? 10.0 : 12.0;
      ...
      std::cerr << "[AlignmentConfig] Enabled tensor alignment correction"
                ...
                << " tolerance="
                << road_cfg.tracing.tensor_alignment_tolerance_degrees << "deg"
...
std::vector<Core::IntersectionTemplate>
CityGenerator::getLastIntersectionTemplates() const {
  return road_generator_.getIntersectionTemplates();
}

// Generates district polygons from the road graph, then enriches district
// metadata (type/desirability) from constraint-derived environmental signals.
std::vector<District>
CityGenerator::classifyDistricts(const fva::Container<Road> &roads,
                                 const WorldConstraintField *constraints,
                                 GenerationContext *context) {
  Urban::DistrictGenerator::Config district_cfg;
  district_cfg.grid_resolution = std::clamp(
      static_cast<int>(std::sqrt(std::max<size_t>(4, roads.size() / 4))), 4,
      14);
  district_cfg.max_districts = config_.max_districts;

  Bounds bounds;
  bounds.min = Vec2(0.0, 0.0);
  bounds.max = Vec2(static_cast<double>(config_.width),
                    static_cast<double>(config_.height));
  auto districts =
      Urban::DistrictGenerator::generate(roads, bounds, district_cfg);
  ...
  if (constraints != nullptr && constraints->isValid()) {
    for (auto &district : districts) {
      ...
      const float flood_penalty =
          constraints->sampleFloodMask(centroid) > 0u ? 0.18f : 0.0f;
      // Simple normalized desirability blend used by downstream heuristics/UI.
      district.desirability = std::clamp(
          1.0f - (slope / 45.0f) + nature * 0.25f - flood_penalty, 0.0f, 1.0f);
    }
  }
...
std::vector<BlockPolygon>
CityGenerator::generateBlocks(const std::vector<District> &districts,
                              GenerationContext *context) {
  if (ShouldAbort(context)) {
    return {};
  }
  ...
  // form genuine block boundaries.  The threshold (>5 districts, non-empty
  // roads) avoids the extra graph-build cost for trivially small cities.
  const bool has_complex_road_network =
      districts.size() > 5 && cache_.roads.size() > 0;
  ...
  // cached road polylines and pass it to BlockGenerator so
  // PolygonFinder::fromGraph can filter district blocks to those actually
  // backed by road network coverage. This eliminates the previous silent no-op
  // where both branches of prefer_road_cycles called fromDistricts regardless
  // of the config value.
...
// Generates lots from roads/districts/blocks and tunes lot sizing by site
// profile.
std::vector<LotToken> CityGenerator::generateLots(
    const fva::Container<Road> &roads, const std::vector<District> &districts,
    const std::vector<BlockPolygon> &blocks, const SiteProfile *profile,
    GenerationContext *context) {
  if (ShouldAbort(context)) {
    return {};
  }
  ...
  if (profile != nullptr) {
    const float buildable_scale =
        std::clamp(profile->buildable_fraction, 0.15f, 1.0f);
    lot_cfg.max_lots = std::max<uint32_t>(
        600u, static_cast<uint32_t>(static_cast<float>(lot_cfg.max_lots) *
                                    buildable_scale));
  ...
  return Urban::LotGenerator::generate(roads, districts, blocks, lot_cfg,
                                       config_.seed,
                                       cache_.intersection_templates);
}

// Generates building sites from lot inventory with profile-specific caps.
siv::Vector<BuildingSite>
CityGenerator::generateBuildings(const std::vector<LotToken> &lots,
  ...
  if (profile != nullptr && profile->mode == GenerationMode::ConservationOnly) {
    site_cfg.max_buildings =
        std::max<uint32_t>(1000u, config_.max_buildings / 2u);
  }
...
void CityGenerator::rasterizeDistrictZonesToTexture(
  ...
  for (const auto &district : districts) {
    ...
    const uint8_t zone_value =
        static_cast<uint8_t>(static_cast<uint8_t>(district.type) + 1u);

    // Iterate only over district AABB in pixel space, then apply exact polygon
    // test.
    for (int y = y0; y <= y1; ++y) {
      for (int x = x0; x <= x1; ++x) {
...
siv::Vector<BuildingSite> CityGenerator::filterBuildingsByTexture(
    ...
    const std::vector<LotToken> &lots, const std::vector<District> &districts,
  ...
  for (const auto &building : buildings) {
    ...
    const bool blocked = Core::Data::DecodeMaterialNoBuild(material);
    if (blocked ||
        slope > static_cast<float>(config_.terrain.max_buildable_slope_deg)) {
      continue;
    }
      ...
      if (lot_it != lot_to_district.end()) {
        ...
        if (district_it != district_type_by_id.end()) {
          const uint8_t expected = static_cast<uint8_t>(
              static_cast<uint8_t>(district_it->second) + 1u);
...
// Ensures global texture-space exists and is synchronized with current
// world/constraint data. When constraint data is available,
// height/material/slope layers are re-seeded from it.
void CityGenerator::initializeTextureSpaceIfNeeded(
  ...
  world_bounds.max = Vec2(static_cast<double>(config_.width),
                          static_cast<double>(config_.height));
...
#if !defined(NDEBUG)
    [[maybe_unused]] const double constraints_world_width =
        static_cast<double>(constraints->width) * constraints->cell_size;
    [[maybe_unused]] const double constraints_world_height =
        static_cast<double>(constraints->height) * constraints->cell_size;
    [[maybe_unused]] const double tolerance =
        std::max(1.0, constraints->cell_size);
    assert(std::abs(constraints_world_width -
                    static_cast<double>(config_.width)) <= tolerance);
    assert(std::abs(constraints_world_height -
                    static_cast<double>(config_.height)) <= tolerance);
#endif
  }
  if (resolution <= 0 && config_.cell_size > 0.0) {
    const double meters =
        static_cast<double>(std::max(config_.width, config_.height));
  ...
  if (constraints != nullptr && constraints->isValid() &&
    ...
    // The upstream terrain layers are now authoritative; tensor remains dirty
    // until regenerated.
...
void CityGenerator::updateGridAnalytics(const fva::Container<Road> &roads) {
  ...
  if (cache_.world_constraints.isValid()) {
    cache_.grid_quality.terrain_adaptation =
        cache_.site_profile.average_buildable_slope;
  }
}
} // namespace RogueCity::Generators
