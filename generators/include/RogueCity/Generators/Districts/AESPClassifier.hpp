...
public:
  ...
  /// Compute AESP scores from road types (legacy enum-only path)
  [[nodiscard]] static AESPScores computeScores(RoadType primary,
                                                RoadType secondary);

  /// Compute AESP scores from fully semantic ASAM Magic Splines
  [[nodiscard]] static AESPScores computeScores(const Core::Road &primary,
                                                const Core::Road &secondary);

  /// Compute AESP scores directly from an ASAM Junction archetype complexity
  [[nodiscard]] static AESPScores
  computeJunctionScores(const Core::IntersectionTemplate &junction);

  /// Classify district type from AESP scores (research formulas)
  [[nodiscard]] static DistrictType
  classifyDistrict(const AESPScores &scores,
                   const ScoringProfile &profile = ScoringProfile::Urban());

  /// Classify district directly from lot token
  [[nodiscard]] static DistrictType
  classifyLot(const LotToken &lot,
              const ScoringProfile &profile = ScoringProfile::Urban());
