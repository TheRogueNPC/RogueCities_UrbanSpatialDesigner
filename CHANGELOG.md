...
- **Rust Client Bindings Fix**: Fixed `rc-db-generate` command in the `dev-shell.ps1` to correctly target the `rc_db_client/src/bindings/` directory, resolving a critical Rust FFI compilation error.
- **SpacetimeDB Server Schema (Rust)**: Created `server/src/lib.rs` with `Axiom` and `GenerationIntent` tables and `place_axiom`/`clear_axioms` reducers targeting SpacetimeDB v2.0.3.
- **Rust FFI Bridge (`rc_db_client`)**: Implemented `extern "C"` ABI wrapper crate over the SpacetimeDB Rust SDK, exposing `rc_db_connect`, `rc_db_disconnect`, `rc_db_place_axiom`, and `rc_db_clear_axioms` as C-linkage functions for C++ consumption.
- **CMake Cargo Integration**: Added `rc_db_client/CMakeLists.txt` that invokes `cargo build` from MSBuild and imports the resulting `.lib` as a static library target with Windows system library dependencies (ws2_32, bcrypt, etc.).
- **C++ SpacetimeClient Wrapper**: Created `core/include/RogueCity/Core/Database/SpacetimeClient.hpp` — a static C++ class wrapping the Rust FFI C calls with `std::string_view` interfaces.
- **DatabaseComputeWorker (App Layer)**: Created `app/src/Integration/DatabaseComputeWorker.cpp` — a background thread that connects to SpacetimeDB, polls a dirty flag for DB change notifications, converts `EditorAxiom` → `CityGenerator::AxiomInput`, and invokes the generation pipeline.
- **Headless Entry Point Wiring**: Updated `visualizer/src/main.cpp` to instantiate and start the `DatabaseComputeWorker` on launch, establishing the "Three-Client" architecture for headless compute.
- **Generators Clipper2 Fix**: Made `Clipper2::Clipper2` link conditional in `generators/CMakeLists.txt` since Clipper2 is compiled from source in `3rdparty/Clipper2`, not via vcpkg.
- **Data Viz Readiness — Full Entity Tables**: Expanded server schema from 2 to 7 tables (Axiom, Road, District, Lot, Building, GenerationStats, GenerationIntent) with upsert-style `push_*` reducers and `clear_generated` bulk wipe. Extended `rc_db_client` FFI with 6 new `extern "C"` push functions using flat-array Vec2 marshaling. Expanded `SpacetimeClient.hpp` with full push API surface for headless compute→DB→data-viz flow.
- **Scalable Generator→DB Pipeline**: Wired `DatabaseComputeWorker::RunGeneratorPass()` to invoke `CityGenerator::generate()`, flatten polygon data to interleaved doubles, and push all roads/districts/lots/buildings to SpacetimeDB with generation serial tracking, 200ms debounce throttling, and per-1000-entity batch progress logging for metro-scale scalability.
- **Dev Console SpacetimeDB Commands**: Added `rc-db-publish`, `rc-db-generate`, `rc-db-status`, `rc-db-logs`, `rc-db-clear`, `rc-db-sql`, `rc-db-build-ffi` to `dev-shell.ps1` for full database lifecycle management from the hyper-reactive console.


- **Full panel interaction migration to `RC_UI::API`**: Updated legacy panel sources to route interactive/widget and common layout actions (`Button`, `Checkbox`, `Selectable`, `Input*`, `Drag*`, `Slider*`, `Combo*`, `MenuItem`, popup/menu helpers, `SetNextItemWidth`, `TextDisabled`, `Spacing`, `Separator`, `SameLine`, etc.) through the canonical wrapper API.
- **Expanded token API surface**: Extended `visualizer/src/ui/api/rc_imgui_api.h/.cpp` with missing action wrappers and added a global migration alias (`namespace API = RC_UI::API`) so non-`RC_UI` panel namespaces can still consume the canonical API consistently.
- **Strict regression prevention**: Hardened `tools/check_imgui_contracts.py` to fail builds on direct raw panel widget calls (`ImGui::Button/Checkbox/...`) and require wrapper usage; this makes wrapper-first behavior mandatory instead of advisory.
- **Unified fold-node visual style across settings/dev/AI panels**: Migrated `rc_panel_dev_shell.cpp`, `rc_panel_ui_settings.cpp`, `rc_panel_ai_console.cpp`, `rc_panel_ui_agent.cpp`, and `rc_panel_city_spec.cpp` to shared `API::SectionHeader(...)` nodes so these panels now use the same cockpit folding treatment as Inspector.

## [Unreleased] - 2026-03-06 (UI Compliance Contracts Refresh)
- **Wrapper-aware ImGui contract enforcement**: Updated `tools/check_imgui_contracts.py` to treat `RC_UI::API` calls as first-class contract inputs (DrawContent top-level guard now includes `API::BeginPanel`, loop/duplicate ID heuristics now scan both `ImGui::` and `API::` interactive calls).
- **Canonical migration guardrails**: Added explicit regression checks for canonical API-migrated panels (currently `rc_panel_texture_editing.cpp`) so raw widget calls (`ImGui::Button`, `ImGui::DragFloat`, etc.) fail compliance and preserve wrapper-first architecture.
- **Compliance docs alignment**: Updated `docs/20_specs/ui-faq-compliance-matrix.md`, `docs/knowledge-base/notes/ui-migration-compliance-and-automation.md`, and `AI/collaboration/imgui_coding_standard.md` to reflect the canonical `visualizer/src/ui/api/rc_imgui_api.*` contract and `API::Mutate` escape-hatch policy.

## [Unreleased] - 2026-03-05 (ImGui Developer Tools — Debug Productivity + PVS-Studio)
- **ImGui Debug Productivity section (Dev Shell)**: Added "ImGui Debug Productivity" collapsing header to the Dev Shell panel with a live ALT-key breakpoint indicator (showing whether `io.KeyAlt` is held — the canonical Dear ImGui breakpoint filter pattern) and a Natvis file status check/copy-path button for `3rdparty/imgui/misc/debuggers/imgui.natvis`.
- **Test Suite section (Dev Shell)**: Added "Test Suite (CTest)" collapsing header with async Core/Gen/App/All buttons that launch the corresponding `rc-tst-*` dev-shell commands via a new `RunDevShellCmdAsync` helper; output is captured in a scrollable child window.
- **PVS-Studio Static Analysis section (Dev Shell)**: Added "PVS-Studio Static Analysis" collapsing header with configurable PVS-Studio dir, output dir, project name, and open-report toggle. Run button calls `rc-pvs-studio` via dev-shell; button disabled when `PVS-Studio_Cmd.exe` is absent at configured path.
- **`rc-pvs-studio` dev-shell function**: PowerShell adaptation of the Dear ImGui `run_pvs_studio.bat` recipe. Locates `.vcxproj` under `RC_BUILD_DIR`, runs PVS-Studio analysis, converts `.plog` to Html/FullHtml/Txt/Totals with `GA:1,2;OP:1` severity filter, prints totals, and optionally opens the HTML report. Listed in `rc-help` under Advanced Tooling.
...
- **Determinism + Contract Coverage**: Added integration/unit tests for scaffold/no-axiom generation, ROADG connectivity constraints, CRS golden contract behavior, GeoPackage round-trip (gated), FBCZ constraint enforcement, and policy auditor reporting.
- **Windows DevShell Build Reliability**: Hardened `tools/dev-shell.ps1` to import `.vscode/vsdev-init.cmd` into the active PowerShell process, expose toolchain readiness (`RC_DEVSHELL_MSBUILD_READY`), and normalize CMake/Ninja/LLVM path discovery.
- **Preset Matrix Alignment (MSBuild default + Ninja/Clang variants)**: Reworked `CMakePresets.json`, `.vscode/settings.json`, `build_and_run_gui.ps1`, and `tools/env_doctor.py` so default dev flow uses `RogueCities Dev PowerShell` + `dev/gui-release` (MSBuild), while `dev-vs2026`, `dev-ninja-msvc`, and `dev-ninja-clang` paths are explicit and health-checkable.
- **Build/Debug Unblock Fixes**: Fixed `rc_opendrive` install-interface include export contract (`BUILD_INTERFACE`), added missing direct `FastVectorArray` includes for thesis pipeline headers, repaired a `ROGUECITY_USE_BOOST_POLYGON` preprocessor mismatch fallback in `MajorConnectorGraph.cpp`, and corrected `BehaviorTreeAuditors` FVA container usage (`size()` vs `empty()`).
- **GenerationCoordinator Compile Repair**: Restored phase string mapping via `GenerationCoordinator::PhaseName(...)`, resolving app-layer compile errors during phase transition logging and enabling successful `gui-release` linking.
- **Master Panel Navigation Recovery**: Restored the top-level "Indices | Controls | Tools | System" tab bar in `RcMasterPanel.cpp`, providing clear macro-navigation across the editor suites.
- **Axiom Library Sidebar Integration**: Transplanted the Axiom Library icon widget to `rc_panel_axiom_editor.cpp`, ensuring it strictly follows the Cockpit Doctrine and only renders in the sidebar when `Editing_Axioms` is active.
- **Unified Tool & Mode Orchestration**: Consolidated mode switching into high-visibility icons in the Master Panel header and sub-tool access into a dynamic `SUB-TOOLS` grid in the Tools panel, eliminating UI redundancy while preserving full feature access.
- **Cockpit Doctrine UI Components**: Introduced `DrawToolIcon` and `DrawToolLibrarySwitcher` as reusable, icon-driven UI primitives in `rc_ui_components.h`, enforcing a unified Y2K visual language across all tool-selection surfaces.
