/**
 * @file ThemeManager.h
 * @brief Multi-theme system for Y2K Cockpit UI in RogueCities Urban Spatial
 * Designer.
 *
 * Provides management and application of UI themes for ImGui-based interfaces.
 * Supports built-in and custom themes, loading/saving from JSON, and runtime
 * editing.
 *
 * @namespace RogueCity::UI
 *
 * @struct ThemeProfile
 *   Represents a theme color profile with 12 key tokens for accents,
...
public:
  ...
  /// Helper to resolve string tokens to actual ImU32 from the active
  /// ThemeProfile
  ImU32 ResolveToken(const std::string &token) const {
    const auto &theme = GetActiveTheme();
    if (token == "color.ui.primary")
      return theme.primary_accent;
    if (token == "color.ui.secondary")
      return theme.secondary_accent;
    if (token == "color.ui.success")
      return theme.success_color;
    if (token == "color.ui.warning")
      return theme.warning_color;
    if (token == "color.ui.error")
      return theme.error_color;
    if (token == "color.ui.border")
  ...
  bool SaveThemeToFile(const ThemeProfile &profile,
...
ThemeProfile Enterprise();       // Near-monochrome, high contrast, minimal — #0A0A0F bg, #C8D8E0 text
ThemeProfile Planner();          // Clean cyan-on-dark, tabular feel — #060B10 bg, #68D6D8 primary
} // namespace Themes
