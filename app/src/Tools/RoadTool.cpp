...
namespace {
    Tools::SplineManipulatorParams GetRoadParams(const Core::Editor::GlobalState& gs, const PrimaryViewport* viewport) {
...
void RoadTool::on_mouse_down(const Core::Vec2& world_pos) {
    ...
    auto* road = gs.selection.selected_road ? Core::Editor::FindRoadMutable(gs, gs.selection.selected_road->id) : nullptr;
    
    if (road) {
        const auto params = GetRoadParams(gs, viewport_);
        if (Tools::SplineManipulator::TryBeginDrag(road->points, world_pos, params, spline_state_)) {
            spline_state_.entity_id = road->id;
        } else if (gs.tool_runtime.road_spline_subtool == Core::Editor::RoadSplineSubtool::Pen) {
...
void RoadTool::on_mouse_move(const Core::Vec2& world_pos) {
    ...
    if (road) {
        const auto params = GetRoadParams(gs, viewport_);
        if (Tools::SplineManipulator::UpdateDrag(road->points, world_pos, params, spline_state_)) {
            gs.dirty_layers.MarkDirty(Core::Editor::DirtyLayer::Roads);
        }
    }
}
...
void RoadTool::on_right_click(const Core::Vec2& world_pos) {
    ...
    if (road) {
        const auto params = GetRoadParams(gs, viewport_);
        if (Tools::SplineManipulator::RemoveVertex(road->points, world_pos, params)) {
            gs.dirty_layers.MarkDirty(Core::Editor::DirtyLayer::Roads);
        }
    }
}
