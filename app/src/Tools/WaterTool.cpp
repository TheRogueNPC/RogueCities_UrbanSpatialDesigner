...
GetWaterParams(const Core::Editor::GlobalState &gs,
               const PrimaryViewport *viewport, const Core::WaterBody &water) {
  ...
  const auto policy = Tools::ResolveGeometryPolicy(
      gs, Core::Editor::EditorState::Editing_Water);
  ...
  // Use GeometryPolicy for falloff modes
  ...
  if (gs.tool_runtime.water_subtool == WaterSubtool::Erode ||
    ...
    params.falloff_radius =
        static_cast<float>(policy.water_falloff_radius_world);
    params.falloff_strength =
        (gs.tool_runtime.water_subtool == WaterSubtool::Erode) ? 0.72f : 0.9f;
  }
...
const char *WaterTool::tool_name() const { return "WaterTool"; }
...
void WaterTool::on_mouse_down(const Core::Vec2 &world_pos) {
  ...
  uint32_t water_id =
      (primary && primary->kind == Core::Editor::VpEntityKind::Water)
          ? primary->id
          : 0u;
  auto *water =
      water_id != 0 ? Core::Editor::FindWaterMutable(gs, water_id) : nullptr;

  if (water) {
    const auto params = GetWaterParams(gs, viewport_, *water);
    if (Tools::SplineManipulator::TryBeginDrag(water->boundary, world_pos,
                                               params, spline_state_)) {
      spline_state_.entity_id = water->id;
    } else if (gs.tool_runtime.water_spline_subtool ==
               Core::Editor::WaterSplineSubtool::Pen) {
...
void WaterTool::on_mouse_move(const Core::Vec2 &world_pos) {
  ...
  if (water) {
    const auto params = GetWaterParams(gs, viewport_, *water);
    if (Tools::SplineManipulator::UpdateDrag(water->boundary, world_pos, params,
                                             spline_state_)) {
...
void WaterTool::on_right_click(const Core::Vec2 &world_pos) {
  ...
  if (water) {
    const auto params = GetWaterParams(gs, viewport_, *water);
    if (Tools::SplineManipulator::RemoveVertex(water->boundary, world_pos,
                                               params)) {
      gs.dirty_layers.MarkDirty(Core::Editor::DirtyLayer::ViewportIndex);
    }
  }
