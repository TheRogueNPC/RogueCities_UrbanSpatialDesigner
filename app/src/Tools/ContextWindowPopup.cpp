...
void ContextWindowPopup::update() {
  ...
  ImGui::PushStyleColor(ImGuiCol_Border,
                        IM_COL32(255, 200, 0, 255)); // Amber warning stripe
  ImGui::PushStyleColor(ImGuiCol_WindowBg,
                        IM_COL32(20, 20, 20, 240)); // Dark background

  if (ImGui::BeginPopup("##ContextValueInput",
...
bool ContextWindowPopup::is_open() const { return is_open_; }
...
void ContextWindowPopup::render_popup() {
  ...
  const bool enter_pressed = ImGui::InputText(
      "##ValueInput", input_buffer_, sizeof(input_buffer_),
...
void ContextWindowPopup::apply_with_interpolation(float new_value) {
  if (on_apply_) {
    ...
    // TODO[Phase7-UX-001]: Trigger smooth interpolation animation (0.3s
    // ease-in-out) This would be handled by the AxiomVisual's ring update logic
  }
}
