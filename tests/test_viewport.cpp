...
bool approx_equal(double a, double b, double epsilon = 0.1) {
  return std::abs(a - b) < epsilon;
}

bool approx_equal(const Core::Vec2 &a, const Core::Vec2 &b,
                  double epsilon = 0.1) {
...
void test_coordinate_conversion() {
  ...
  // Test 1: Round-trip conversion (world -> screen -> world)
  {
    ...
    if (approx_equal(original_world, converted_world, 1.0f)) {
      ...
      std::cout << "   Original: (" << original_world.x << ", "
                << original_world.y << ")\n";
      std::cout << "   Screen:   (" << screen.x << ", " << screen.y << ")\n";
      std::cout << "   Converted: (" << converted_world.x << ", "
                << converted_world.y << ")\n";
    } else {
      std::cout << "? Round-trip conversion: FAIL\n";
      std::cout << "   Error: " << original_world.distanceTo(converted_world)
                << " units\n";
    }
  }
...
void test_viewport_sync() {
  ...
  // Test 2: Smooth sync (lerp factor 0.2)
  {
    ...
    const double error = primary_xy.distanceTo(minimap_xy);
    if (error < 50.0f) { // Within 50 units after smoothing
      std::cout << "? Smooth sync: PASS (error: " << error << " units)\n";
    } else {
      std::cout << "? Smooth sync: FAIL (error too large: " << error
                << " units)\n";
    }
  }
