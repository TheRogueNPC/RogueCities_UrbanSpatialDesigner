...
void CitySpecPanel::RenderContent() {
    ...
    if (API::SectionHeader("City Description", RC_UI::UITokens::MagentaHighlight, true)) {
        RenderBusyIndicator(m_processing, m_busyTime);
        
        ImGui::Text("Describe your city:");
        API::InputTextMultiline("##desc", m_descBuffer, sizeof(m_descBuffer), ImVec2(-1, 120));
        uiint.RegisterWidget({"text", "Description", "city_spec.description", {"input", "ai", "city_spec"}});
        
        API::Combo("Scale", &m_scaleIndex, scales, 4);
    ...
    if (hasSpecCopy) {
        API::Separator();
        if (API::SectionHeader("Generated Specification", RC_UI::UITokens::CyanAccent, true)) {
