...
RogueCity::Core::Road *FindRoad(RogueCity::Core::Editor::GlobalState &gs,
                                uint32_t id) {
...
void RenderSubtoolToggle(const char *label, RoadEditorSubtool tool) {
  ...
  if (is_active) {
    ...
    ImGui::PushStyleColor(
        ImGuiCol_Button, ImGui::ColorConvertU32ToFloat4(UITokens::Transparent));
  }
...
public:
  SetRoadTypeCommand(uint32_t road_id, RogueCity::Core::RoadType before,
                     RogueCity::Core::RoadType after)
      : road_id_(road_id), before_(before), after_(after) {}

  void Execute() override { Apply(after_); }

  void Undo() override { Apply(before_); }

  const char *GetDescription() const override { return "Road Type"; }
...
void DispatchRoadTypeChange(uint32_t road_id, RogueCity::Core::RoadType before,
                            RogueCity::Core::RoadType after) {
  if (before == after) {
    return;
  }
  ...
  if (Components::DrawSectionHeader(
    ...
    if (gs.selection.selected_road) {
      ...
      if (road) {
        ...
        ImGui::Text("Length: %.2f m (approx)",
                    road->points.size() > 1 ? (road->points.size() - 1) * 10.0f
                                            : 0.0f);
        const auto type_name = magic_enum::enum_name(road->type);
        ImGui::Text("Flow Capacity (Type): %s", type_name.data());
      } else {
        ImGui::TextColored(
            ImGui::ColorConvertU32ToFloat4(UITokens::TextDisabled),
            "Selected road not found.");
      }
    } else {
...
void Draw(float dt) {
  ...
  if (!Components::BeginTokenPanel("Road Network Editor",
                                   UITokens::AmberGlow)) {
