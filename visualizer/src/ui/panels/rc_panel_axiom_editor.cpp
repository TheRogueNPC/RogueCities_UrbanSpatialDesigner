...
enum class MinimapLOD : uint8_t { Strategic = 0, Tactical, Detail };
...
static constexpr float kMinimapWorldSize =
    2000.0f; // World bounds for minimap (2km)
...
void DrawAxiomModeStatus(const Preview &preview, ImVec2 pos) {
  ...
  if (phase == Preview::GenerationPhase::InitStreetSweeper) {
    ...
    const int visible =
        std::clamp(static_cast<int>(reveal * static_cast<float>(len)), 0, len);
  ...
  if (phase == Preview::GenerationPhase::Sweeping) {
    ...
    ImGui::TextColored(TokenColorF(UITokens::AmberGlow), "_SWEEPING  %.0f%%",
                       p * 100.0f);
    return;
  }
  ...
  ImGui::TextColored(TokenColorF(UITokens::TextPrimary, 242u),
                     "AXIOM MODE ACTIVE");
}
...
BuildNonAxiomContextCue(const RogueCity::Core::Editor::GlobalState &gs) {
  ...
  case ToolDomain::Paths:
    ...
    case RogueCity::Core::Editor::RoadSplineSubtool::Pen:
      return "Cue: Freehand drag. Hold Shift for point mode. Ctrl+DoubleClick "
             "closes.";
    ...
    case RoadSubtool::Strengthen:
      return "Cue: LMB near road upgrades hierarchy.";
    case RoadSubtool::Disconnect:
      return "Cue: LMB on vertex disconnects/splits road.";
    case RoadSubtool::Stub:
      return "Cue: LMB drops short road stubs quickly.";
    case RoadSubtool::Curve:
      return "Cue: Pen mode + TinySpline smoothing.";
    case RoadSubtool::Grid:
      return "Cue: Pen mode snapped to grid.";
    default:
      return "Cue: LMB pen/add, drag vertices in spline subtools.";
    }
  case ToolDomain::District:
  case ToolDomain::Zone:
    switch (gs.tool_runtime.district_subtool) {
    case DistrictSubtool::Split:
      return "Cue: Insert mode on edges, drag vertices.";
    case DistrictSubtool::Merge:
      return "Cue: LMB near another district to merge.";
    default:
      return "Cue: LMB place/paint district areas.";
    }
  case ToolDomain::Lot:
    switch (gs.tool_runtime.lot_subtool) {
    case LotSubtool::Slice:
      return "Cue: LMB slices selected lot at cursor.";
    case LotSubtool::Merge:
      return "Cue: LMB nearby lot to merge.";
    case LotSubtool::Align:
      return "Cue: LMB realigns lot to snap/grid.";
    default:
      return "Cue: LMB plots and edits lots.";
    }
  case ToolDomain::Building:
  case ToolDomain::FloorPlan:
  case ToolDomain::Furnature:
    switch (gs.tool_runtime.building_subtool) {
    case BuildingSubtool::Rotate:
      return "Cue: LMB rotates picked building (15deg).";
    case BuildingSubtool::Scale:
      return "Cue: LMB scales picked building (+10%).";
    case BuildingSubtool::Assign:
      return "Cue: LMB cycles building type.";
    default:
      return "Cue: LMB places buildings; gizmo handles transforms.";
    }
  ...
  case ToolDomain::Flow:
    ...
    case RogueCity::Core::Editor::WaterSplineSubtool::Selection:
      return "Cue: Selection picks full water spline; hover highlights "
             "candidate.";
    ...
    case WaterSubtool::Mask:
      return "Cue: LMB toggles shoreline mask on selected body.";
    case WaterSubtool::Erode:
      return "Cue: Drag to erode/expand shoreline radially with falloff.";
    case WaterSubtool::Contour:
      return "Cue: Drag to contour-align edits (dominant axis) with falloff.";
    case WaterSubtool::Flow:
      return "Cue: Drag to advect edits along local tangent flow with falloff.";
    default:
      return "Cue: LMB pen/add anchors, spline tools refine shape.";
    }
...
struct ClearLayerCommand : public RogueCity::App::ICommand {
  ...
  std::vector<RogueCity::App::AxiomPlacementTool::AxiomSnapshot>
      axioms_snapshot{};
    ...
    if (layers & Axioms) {
      ...
      if (auto *axiom_tool = RC_UI::Panels::AxiomEditor::GetAxiomTool()) {
        ...
        for (const auto &axiom : tool_axioms) {
          ...
          for (size_t r = 0; r < snapshot.radial_ring_knob_weights.size();
               ++r) {
            for (size_t k = 0; k < snapshot.radial_ring_knob_weights[r].size();
                 ++k) {
              snapshot.radial_ring_knob_weights[r][k] =
                  axiom->radial_ring_knob_weight(r, k);
            }
          }
...
static std::unique_ptr<RogueCity::App::GenerationCoordinator>
...
void EnsureViewportDerivedIndices(RogueCity::Core::Editor::GlobalState &gs) {
  ...
  const bool has_entities = gs.roads.size() > 0 || gs.districts.size() > 0 ||
                            gs.lots.size() > 0 || gs.buildings.size() > 0 ||
                            gs.waterbodies.size() > 0;

  const bool missing_viewport_index = has_entities && gs.viewport_index.empty();
  const bool missing_spatial_grid =
      has_entities && !gs.render_spatial_grid.IsValid();
  const bool stale_viewport_index = !has_entities && !gs.viewport_index.empty();
  const bool stale_spatial_grid =
      !has_entities && gs.render_spatial_grid.IsValid();
  const bool needs_rebuild =
      gs.dirty_layers.IsDirty(DirtyLayer::ViewportIndex) ||
      missing_viewport_index || missing_spatial_grid || stale_viewport_index ||
      stale_spatial_grid;
  ...
  // Spatial picking acceleration can be layered in later without changing this
  // API.
}
...
void Initialize() {
  ...
  s_generation_coordinator =
      std::make_unique<RogueCity::App::GenerationCoordinator>();
  ...
  s_sync_manager = std::make_unique<RogueCity::App::ViewportSyncManager>(
      s_primary_viewport.get(), minimap);

  // Set initial camera position (centered on 1000,1000 city)
  s_primary_viewport->set_camera_position(RogueCity::Core::Vec2(1000.0, 1000.0),
                                          500.0f);
  ...
  s_generation_coordinator->SetOnComplete(
      [minimap](const RogueCity::Generators::CityGenerator::CityOutput &output,
...
RogueCity::App::AxiomPlacementTool *GetAxiomTool() {
  return s_axiom_tool.get();
}
RogueCity::App::RealTimePreview *GetPreview() {
  return s_generation_coordinator ? s_generation_coordinator->Preview()
                                  : nullptr;
}
RogueCity::App::AxiomVisual *GetSelectedAxiom() {
  return s_axiom_tool ? s_axiom_tool->get_selected_axiom() : nullptr;
}
...
bool CanUndo() { return s_axiom_tool && s_axiom_tool->can_undo(); }

bool CanRedo() { return s_axiom_tool && s_axiom_tool->can_redo(); }
...
void SetFlowRate(float flowRate) {
  s_flow_rate = std::clamp(flowRate, 0.1f, 4.0f);
}
...
void ClearLayer(uint32_t layer_mask, const char *description) {
  ...
  if (layer_mask == ClearLayerCommand::All) {
    auto &gs = RogueCity::Core::Editor::GetGlobalState();
    const auto b = RogueCity::Core::Editor::ComputeWorldBounds(
        gs.city_texture_size, gs.city_meters_per_pixel);
    gs.city_boundary = {{b.min.x, b.min.y},
                        {b.max.x, b.min.y},
                        {b.max.x, b.max.y},
                        {b.min.x, b.max.y}};
...
void ClearAxioms() { ClearLayer(ClearLayerCommand::Axioms, "Clear Axioms"); }

void ClearWater() { ClearLayer(ClearLayerCommand::Water, "Clear Water"); }

void ClearRoads() { ClearLayer(ClearLayerCommand::Roads, "Clear Roads"); }

void ClearDistricts() {
  ClearLayer(ClearLayerCommand::Districts, "Clear Districts");
}

void ClearLots() { ClearLayer(ClearLayerCommand::Lots, "Clear Lots"); }
...
static bool BuildInputs(
  ...
  const auto world_bounds = RogueCity::Core::Editor::ComputeWorldBounds(
      gs.city_texture_size, gs.city_meters_per_pixel);
  const double world_extent_raw =
      std::max(world_bounds.max.x - world_bounds.min.x,
               world_bounds.max.y - world_bounds.min.y);
  const int world_extent =
      std::clamp(static_cast<int>(std::lround(world_extent_raw)), 500, 8192);
...
bool CanGenerate() {
  if (!s_generation_coordinator || !s_axiom_tool)
    return false;
...
void ForceGenerate() {
  ...
  if (!BuildInputs(inputs, cfg))
    return;

  auto &gs = RogueCity::Core::Editor::GetGlobalState();
  const auto plan =
      RC_UI::Viewport::BuildPlacementGenerationPlan(gs.dirty_layers);
  if (plan.use_incremental &&
      !gs.dirty_layers.IsDirty(RogueCity::Core::Editor::DirtyLayer::Axioms)) {
    s_generation_coordinator->ForceRegenerationIncremental(
        inputs, cfg, plan.dirty_stages,
    ...
    s_generation_coordinator->ForceRegeneration(
        inputs, cfg, RogueCity::App::GenerationDepth::FullPipeline,
        RogueCity::App::GenerationRequestReason::ForceGenerate);
  }
...
const char *GetRogueNavModeName() {
  ...
  case MinimapMode::Soliton:
    return "SOLITON";
  case MinimapMode::Reactive:
    return "REACTIVE";
  case MinimapMode::Satellite:
    return "SATELLITE";
  case MinimapMode::Disabled:
    return "OFF";
  default:
    return "OFF";
  }
...
const char *GetRogueNavFilterName() {
  ...
  case RogueNavAlert::Normal:
    return "NORMAL";
  case RogueNavAlert::Caution:
    return "CAUTION";
  case RogueNavAlert::Evasion:
    return "EVASION";
...
bool IsMinimapManualLODOverride() { return !s_minimap_auto_lod; }
...
int GetMinimapManualLODLevel() { return MinimapLODLevel(s_minimap_lod); }
...
bool IsMinimapAdaptiveQualityEnabled() { return s_minimap_adaptive_quality; }
...
static ImU32 GetNavAlertColor() {
  switch (s_nav_alert_level) {
  case RogueNavAlert::Alert:
    return UITokens::ErrorRed; // Red - Detected
  case RogueNavAlert::Evasion:
    return UITokens::AmberGlow; // Orange - Tracked
  case RogueNavAlert::Caution:
    return UITokens::YellowWarning; // Yellow - Suspicious
  case RogueNavAlert::Normal:
  default:
    return UITokens::InfoBlue; // Blue - All clear
  }
}

// Convert world coordinates to minimap UV space [0,1]
static ImVec2 WorldToMinimapUV(const RogueCity::Core::Vec2 &world_pos,
                               const RogueCity::Core::Vec2 &camera_pos) {
  // World space relative to camera
  const double world_span = static_cast<double>(kMinimapWorldSize) *
                            static_cast<double>(s_minimap_zoom);
  ...
  return ImVec2(static_cast<float>(0.5 + rel_x),
                static_cast<float>(0.5 + rel_y));
}

// Convert minimap pixel coords to world coordinates
static RogueCity::Core::Vec2
MinimapPixelToWorld(const ImVec2 &pixel_pos, const ImVec2 &minimap_pos,
                    const RogueCity::Core::Vec2 &camera_pos) {
  return RC_UI::Viewport::MinimapPixelToWorld(
      RogueCity::Core::Vec2(pixel_pos.x, pixel_pos.y),
      RogueCity::Core::Vec2(minimap_pos.x, minimap_pos.y), kMinimapSize,
      camera_pos, kMinimapWorldSize, s_minimap_zoom);
}
...
ClampMinimapCameraToConstraints(const RogueCity::Core::Vec2 &camera_pos,
  ...
  if (!gs.config.feature_camera_bounce_clamp) {
    ...
    return RC_UI::Viewport::ClampToWorldConstraints(camera_pos,
  ...
  if (TryCityBoundaryBounds(gs, min_bound, max_bound)) {
    return RC_UI::Viewport::ClampToBoundsWithSpring(
        camera_pos, min_bound, max_bound, delta_seconds,
  ...
  return RC_UI::Viewport::ClampToWorldConstraintsWithSpring(
      camera_pos, gs.world_constraints, delta_seconds,
      gs.tool_runtime.viewport_clamp_overscroll,
      gs.tool_runtime.viewport_clamp_velocity,
      gs.tool_runtime.viewport_clamp_active);
}
...
static MinimapLOD ComputeAutoMinimapLOD(float viewport_zoom,
  ...
  // Secondary modifier: minimap zoom nudges one level, but never dominates
  // viewport zoom.
...
static void UpdateMinimapFPSPressure(float current_fps) {
  ...
  const float fps_sum = std::accumulate(s_minimap_fps_history.begin(),
                                        s_minimap_fps_history.end(), 0.0f);
  const float avg_fps =
      fps_sum / static_cast<float>(s_minimap_fps_history.size());
...
static int
CountDirtyTextureLayers(const RogueCity::Core::Editor::GlobalState &gs) {
  ...
  int dirty = 0;
  for (int i = 0;
       i < static_cast<int>(RogueCity::Core::Data::TextureLayer::Count); ++i) {
...
ComputeAdaptiveMinimapLOD(MinimapLOD base_lod,
  ...
  // Dirty-pressure trigger: >3 dirty texture layers while active
  // painting/terrain edits.
...
enum class RoadDetailClass : uint8_t { Arterial = 0, Collector = 1, Local = 2 };
...
static void RenderMinimapSelectionHighlights(
  ...
  for (const auto &item : gs.selection_manager.Items()) {
    ...
    const bool is_primary = primary != nullptr && primary->kind == item.kind &&
                            primary->id == item.id;
    ...
    const ImVec2 screen_pos(minimap_pos.x + uv.x * kMinimapSize,
...
static ImU32 DistrictColor(RogueCity::Core::DistrictType type, MinimapLOD lod) {
  ...
  case RogueCity::Core::DistrictType::Residential:
    return TokenColor(UITokens::GreenHUD, a);
  case RogueCity::Core::DistrictType::Commercial:
    return TokenColor(UITokens::InfoBlue, a);
  case RogueCity::Core::DistrictType::Industrial:
    return TokenColor(UITokens::AmberGlow, a);
  case RogueCity::Core::DistrictType::Civic:
    return TokenColor(UITokens::MagentaHighlight, a);
  case RogueCity::Core::DistrictType::Mixed:
  default:
    return TokenColor(UITokens::TextSecondary, a);
  }
}

static void
RenderHeightOverlay(ImDrawList *draw_list,
  ...
  for (int py = 0; py < static_cast<int>(kMinimapSize); py += sample_step) {
    for (int px = 0; px < static_cast<int>(kMinimapSize); px += sample_step) {
      const ImVec2 pixel(minimap_pos.x + static_cast<float>(px),
                         minimap_pos.y + static_cast<float>(py));
      const RogueCity::Core::Vec2 world =
          MinimapPixelToWorld(pixel, minimap_pos, camera_pos);
      ...
      const ImU32 color = TokenColor(
          LerpColor(UITokens::InfoBlue, UITokens::AmberGlow, normalized), 48u);
      draw_list->AddRectFilled(
          pixel,
          ImVec2(pixel.x + static_cast<float>(sample_step),
                 pixel.y + static_cast<float>(sample_step)),
          color);
    }
  }
}
...
static void
RenderTensorOverlay(ImDrawList *draw_list,
                    const RogueCity::Core::Data::TextureSpace &texture_space,
                    const RogueCity::Core::Vec2 &camera_pos,
                    const ImVec2 &minimap_pos, MinimapLOD lod) {
  const int lod_stride = OverlayStrideForLOD(lod);
  ...
  for (int py = sample_step / 2; py < static_cast<int>(kMinimapSize);
       py += sample_step) {
    for (int px = sample_step / 2; px < static_cast<int>(kMinimapSize);
         px += sample_step) {
      const ImVec2 pixel(minimap_pos.x + static_cast<float>(px),
                         minimap_pos.y + static_cast<float>(py));
      const RogueCity::Core::Vec2 world =
      ...
      const RogueCity::Core::Vec2 dir =
          texture_space.tensorLayer()
              .sampleBilinearTyped<RogueCity::Core::Vec2>(
                  texture_space.coordinateSystem().worldToUV(world));
      ...
      const ImVec2 a(pixel.x - static_cast<float>(unit.x) * half_len,
                     pixel.y - static_cast<float>(unit.y) * half_len);
      const ImVec2 b(pixel.x + static_cast<float>(unit.x) * half_len,
                     pixel.y + static_cast<float>(unit.y) * half_len);
...
static void RenderMinimapOverlay(ImDrawList *draw_list,
  ...
  // Parent Scale Bar to ROGUENAV mini-map
  if (s_primary_viewport != nullptr) {
    const float base_y = s_minimap_visible
                             ? (minimap_pos.y + kMinimapSize + 16.0f)
                             : (minimap_pos.y + 16.0f);
    const ImVec2 base_pt = ImVec2(minimap_pos.x + kMinimapSize, base_y);

    const float viewport_zoom = s_primary_viewport->world_to_screen_scale(1.0f);
    const float scale_wx = viewport_zoom > 0.0f ? 1.0f / viewport_zoom : 0.0f;
    if (scale_wx > 0.0f) {
      const float target_screen_px = 150.0f; // Target width of scale bar
      const float target_world_m = target_screen_px * scale_wx;

      // SnapNiceMeters logic inline
      auto snap_nice_meters = [](float meters) -> float {
        if (meters <= 0.0f)
          return 0.0f;
      ...
      const float nice_world_m = snap_nice_meters(target_world_m);
      const float actual_screen_px = nice_world_m / scale_wx;

      const ImVec2 right(base_pt.x, base_pt.y);
      const ImVec2 left(base_pt.x - actual_screen_px, base_pt.y);
      const ImU32 color_bg = TokenColor(UITokens::BackgroundDark, 200u);
      const ImU32 color_fg = TokenColor(UITokens::TextPrimary, 255u);

      draw_list->AddLine(ImVec2(left.x, left.y + 1.0f),
                         ImVec2(right.x, right.y + 1.0f), color_bg, 3.0f);
      draw_list->AddLine(ImVec2(left.x, left.y - 4.0f),
                         ImVec2(left.x, left.y + 5.0f), color_bg, 3.0f);
      draw_list->AddLine(ImVec2(right.x, right.y - 4.0f),
                         ImVec2(right.x, right.y + 5.0f), color_bg, 3.0f);

      draw_list->AddLine(left, right, color_fg, 1.0f);
      draw_list->AddLine(ImVec2(left.x, left.y - 3.0f),
                         ImVec2(left.x, left.y + 4.0f), color_fg, 1.0f);
      draw_list->AddLine(ImVec2(right.x, right.y - 3.0f),
                         ImVec2(right.x, right.y + 4.0f), color_fg, 1.0f);

      char scale_buf[32];
      if (nice_world_m >= 1000.0f) {
        std::snprintf(scale_buf, sizeof(scale_buf), "%.2f km",
                      nice_world_m / 1000.0f);
      } else {
        std::snprintf(scale_buf, sizeof(scale_buf), "%.0f m", nice_world_m);
      }

      const ImVec2 text_sz = ImGui::CalcTextSize(scale_buf);
      const ImVec2 text_pos(left.x + (actual_screen_px - text_sz.x) * 0.5f,
                            left.y - text_sz.y - 3.0f);

      draw_list->AddRectFilled(
          ImVec2(text_pos.x - 2.0f, text_pos.y - 1.0f),
          ImVec2(text_pos.x + text_sz.x + 2.0f, text_pos.y + text_sz.y + 1.0f),
          color_bg, 2.0f);
      draw_list->AddText(text_pos, color_fg, scale_buf);
    }
  }

  if (!s_minimap_visible)
    return;
  ...
  draw_list->AddRectFilled(ImVec2(minimap_pos.x + 3, minimap_pos.y + 3),
                           ImVec2(minimap_pos.x + kMinimapSize - 3,
                                  minimap_pos.y + kMinimapSize - 3),
                           TokenColor(UITokens::BackgroundDark, 200u));
  const ImVec2 minimap_clip_min(minimap_pos.x + 3.0f, minimap_pos.y + 3.0f);
  const ImVec2 minimap_clip_max(minimap_pos.x + kMinimapSize - 3.0f,
                                minimap_pos.y + kMinimapSize - 3.0f);
  ...
  if (ImGui::IsItemClicked()) {
    RC_UI::ToggleSystemMap();
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Open System Map");
  }
  ImGui::PopStyleColor();
  ...
  // Shared scene frame (main viewport + minimap) is the authoritative render
  // payload.
  ...
  if (s_axiom_tool) {
    ...
    for (const auto &axiom : axioms) {
      ...
      if (uv.x >= 0.0f && uv.x <= 1.0f && uv.y >= 0.0f && uv.y <= 1.0f) {
        ImVec2 screen_pos = ImVec2(minimap_pos.x + uv.x * kMinimapSize,
                                   minimap_pos.y + uv.y * kMinimapSize);
  ...
  if (output != nullptr) {
    const size_t district_step =
        (active_lod == MinimapLOD::Detail)
            ? 1u
            : ((active_lod == MinimapLOD::Tactical) ? 2u : 6u);
    for (const auto &district : output->districts) {
      ...
      if (active_lod != MinimapLOD::Strategic) {
        for (size_t i = 0; i < district.border.size(); i += district_step) {
          ...
          draw_list->AddLine(ImVec2(minimap_pos.x + uv1.x * kMinimapSize,
                                    minimap_pos.y + uv1.y * kMinimapSize),
                             ImVec2(minimap_pos.x + uv2.x * kMinimapSize,
                                    minimap_pos.y + uv2.y * kMinimapSize),
                             DistrictColor(district.type, active_lod), 1.0f);
        }
      }
      ...
      if (uv.x >= 0.0f && uv.x <= 1.0f && uv.y >= 0.0f && uv.y <= 1.0f) {
        ...
        draw_list->AddCircleFilled(
            screen_pos, (active_lod == MinimapLOD::Strategic) ? 3.5f : 2.5f,
            DistrictColor(district.type, active_lod));
      }
    }

    const auto &roads = output->roads;
    const size_t sample_step =
        (active_lod == MinimapLOD::Detail)
            ? 1u
            : ((active_lod == MinimapLOD::Tactical) ? 3u : 8u);
    ...
    for (auto it = roads.begin(); it != roads.end(); ++it) {
      ...
      if (road.points.size() < 2 ||
          !ShouldRenderRoadForLOD(road.type, active_lod)) {
        continue;
      }
      ...
      for (size_t i = 0; i + 1 < road.points.size(); i += sample_step) {
        ...
        draw_list->AddLine(ImVec2(minimap_pos.x + uv1.x * kMinimapSize,
                           ...
                           ImVec2(minimap_pos.x + uv2.x * kMinimapSize,
                                  minimap_pos.y + uv2.y * kMinimapSize),
                           road_color, road_width);
      }
    }

    if (active_lod == MinimapLOD::Detail) {
      std::unordered_map<uint32_t, const RogueCity::Core::LotToken *>
          lots_by_id;
      ...
      for (const auto &building : output->buildings) {
        ...
        if (lot_it == lots_by_id.end() || lot_it->second == nullptr ||
            lot_it->second->boundary.size() < 3) {
        ...
        for (size_t i = 0; i < footprint.size(); ++i) {
          ...
          draw_list->AddLine(ImVec2(minimap_pos.x + uv1.x * kMinimapSize,
                             ...
                             ImVec2(minimap_pos.x + uv2.x * kMinimapSize,
                                    minimap_pos.y + uv2.y * kMinimapSize),
                             TokenColor(UITokens::TextPrimary, 210u), 1.0f);
        }
      }
      ...
      for (const auto &building : output->buildings) {
        ...
        const ImVec2 screen_pos(minimap_pos.x + uv.x * kMinimapSize,
                                minimap_pos.y + uv.y * kMinimapSize);
        draw_list->AddCircleFilled(screen_pos, 1.4f,
                                   TokenColor(UITokens::TextPrimary, 185u));
      }
    }
  }
  ...
  if (gs.HasTextureSpace()) {
    ...
    if (gs.debug_show_height_overlay) {
      RenderHeightOverlay(draw_list, texture_space, camera_pos, minimap_pos,
                          active_lod);
    }
    if (gs.debug_show_tensor_overlay) {
      RenderTensorOverlay(draw_list, texture_space, camera_pos, minimap_pos,
                          active_lod);
    }
  }

  // Frustum rectangle from current primary viewport zoom.
  // Draw with minimap clip-rect instead of endpoint clamping to avoid edge
  // artifacts.
  const double half_w_world = (static_cast<double>(viewport_size.x) * 0.5) /
                              std::max(0.05f, viewport_zoom);
  const double half_h_world = (static_cast<double>(viewport_size.y) * 0.5) /
                              std::max(0.05f, viewport_zoom);
  const std::array<RogueCity::Core::Vec2, 4> frustum_world = {
      RogueCity::Core::Vec2(camera_pos.x - half_w_world,
  ...
  for (size_t i = 0; i < frustum_world.size(); ++i) {
    ...
    const ImVec2 p1 = ImVec2(minimap_pos.x + uv1.x * kMinimapSize,
                             minimap_pos.y + uv1.y * kMinimapSize);
    const ImVec2 p2 = ImVec2(minimap_pos.x + uv2.x * kMinimapSize,
                             minimap_pos.y + uv2.y * kMinimapSize);
    draw_list->AddLine(p1, p2, UITokens::YellowWarning, 1.5f);
  }

  // Center crosshair (current camera position)
  const ImVec2 center = ImVec2(minimap_pos.x + kMinimapSize / 2.0f,
  ...
  draw_list->AddRect(ImVec2(minimap_pos.x + 6.0f, minimap_pos.y + 6.0f),
                     ImVec2(minimap_pos.x + kMinimapSize - 6.0f,
                            minimap_pos.y + kMinimapSize - 6.0f),
                     TokenColor(UITokens::TextPrimary, 40u), 0.0f, 0, 1.0f);
  ...
  const RogueCity::Core::Vec2 cursor_world =
      mouse_in_minimap ? MinimapPixelToWorld(mouse, minimap_pos, camera_pos)
                       : camera_pos;
...
void DrawAxiomLibraryContent() {
  ...
  ImGui::TextColored(TokenColorF(UITokens::TextPrimary, 217u),
                     "Axiom Library (Ctrl: apply to selection)");
  ...
  const int columns = std::max(
      1,
      static_cast<int>(std::floor((ImGui::GetContentRegionAvail().x + spacing) /
                                  (icon_size + spacing))));
  const auto actions =
  ...
  const auto apply_axiom_type = [&](RogueCity::App::AxiomType axiom_type) {
    if (apply_to_selected) {
      if (selected_axiom != nullptr) {
        ...
        selected_axiom->set_radial_ring_rotation(
  ...
  for (int i = 0; i < static_cast<int>(actions.size()); ++i) {
    ...
    const ImU32 border = is_default ? TokenColor(UITokens::TextPrimary, 220u)
                                    : TokenColor(UITokens::TextSecondary, 160u);

    dl->AddRectFilled(bmin, bmax, bg, 8.0f);
    dl->AddRect(bmin, bmax, border, 8.0f, 0, is_default ? 2.5f : 1.5f);
    RogueCity::App::DrawAxiomIcon(dl, c, icon_size * 0.32f, axiom_type,
                                  info.primary_color);
    ...
    uiint.RegisterWidget(
        {"button",
         action.label,
         std::string("action:") + RC_UI::Tools::ToolActionName(action.id),
         {"tool", "library", "axiom"}});
    ImGui::PopID();
  }
  ...
  for (const auto feature : allowed_features) {
    ...
    if (ImGui::IsItemHovered()) {
      ...
      if (!s_terminal_hover_feature.has_value() ||
  ...
  if (focus_type == RogueCity::App::AxiomType::Radial) {
    ...
    if (ImGui::SliderAngle("Rotate Rings", &ring_rotation, -180.0f, 180.0f,
      ...
      if (selected_axiom != nullptr) {
        ...
        s_axiom_tool->set_default_radial_ring_rotation(focus_type,
                                                       ring_rotation);
      }
    }
  }
  ...
  for (size_t i = 0; i < terminal.alternates.size(); ++i) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Button,
                          TokenColorF(alt_info.primary_color, 185u));
...
void DrawContent(float dt) {
  ...
  // Update viewport sync + generation scheduler through a shared scene
  // controller path.
  RC_UI::Viewport::UpdateSceneController(
      s_scene_frame, RC_UI::Viewport::SceneControllerUpdateInput{
                         dt,
  ...
  const ImVec2 minimap_pos_bounds =
      ImVec2(viewport_pos.x + viewport_size.x - kMinimapSize - kMinimapPadding,
             viewport_pos.y + kMinimapPadding);
  const ImVec2 minimap_max_bounds = ImVec2(minimap_pos_bounds.x + kMinimapSize,
                                           minimap_pos_bounds.y + kMinimapSize);
  const bool minimap_hovered =
      s_minimap_visible && (mouse_pos.x >= minimap_pos_bounds.x &&
                            mouse_pos.x <= minimap_max_bounds.x &&
                            mouse_pos.y >= minimap_pos_bounds.y &&
                            mouse_pos.y <= minimap_max_bounds.y);
  const float chrome_w = 460.0f;
  const float chrome_h =
      gs.tool_runtime.viewport_scene_stats_collapsed ? 64.0f : 124.0f;
  const ImVec2 chrome_min(viewport_pos.x + 12.0f, viewport_pos.y + 12.0f);
  const ImVec2 chrome_max(chrome_min.x + chrome_w, chrome_min.y + chrome_h);
  const bool hud_hovered =
  ...
  const bool allow_viewport_mouse_actions =
      RC_UI::AllowViewportMouseActions(input_gate);
  ...
  s_global_palette_open_lerp =
      std::clamp(s_global_palette_open_lerp, 0.0f, 1.0f);
  ...
  if (axiom_interaction.active && axiom_interaction.has_world_pos) {
    ...
    ImGui::TextColored(
        TokenColorF(UITokens::CyanAccent, 178u), "Mouse World: (%.1f, %.1f)%s",
        axiom_interaction.world_pos.x, axiom_interaction.world_pos.y,
    ...
    if (s_generation_coordinator && s_live_preview && live_preview_allowed &&
      ...
      if (axiom_inputs_dirty) {
        s_generation_coordinator->RequestRegeneration(
            axiom_inputs, config, RogueCity::App::GenerationDepth::AxiomBounds,
        ...
        if (plan.use_incremental && allow_incremental_live_pipeline) {
          s_generation_coordinator->RequestRegenerationIncremental(
              axiom_inputs, config, plan.dirty_stages,
              RogueCity::App::GenerationDepth::FullPipeline,
              RogueCity::App::GenerationRequestReason::LivePreview);
          gs.dirty_layers.MarkAllClean();
          gs.tool_runtime.explicit_generation_pending = false;
  ...
  const char *viewport_status =
      gs.tool_runtime.last_viewport_status.empty()
          ? "idle"
          : gs.tool_runtime.last_viewport_status.c_str();
  const char *active_domain =
      RC_UI::Tools::ToolDomainName(gs.tool_runtime.active_domain);
  const size_t axiom_count =
      s_axiom_tool ? s_axiom_tool->axioms().size() : size_t{0};
  const size_t road_count =
      gs.show_layer_roads ? static_cast<size_t>(gs.roads.size()) : 0u;

  // ---------------------------------------------------------------------
  // Viewport overlays (Minimap, HUD, Warning Ribbon)
  // ---------------------------------------------------------------------
  ...
  if (gs.config.feature_ui_chrome_unification) {
    ...
    draw_list->AddRectFilled(uni_chrome_min, uni_chrome_max,
                             TokenColor(UITokens::BackgroundDark, 180u), 8.0f);
    draw_list->AddRect(uni_chrome_min, uni_chrome_max,
                       TokenColor(UITokens::TextSecondary, 100u), 8.0f, 0,
                       1.2f);

    ImGui::SetCursorScreenPos(
        ImVec2(uni_chrome_min.x + 12.0f, uni_chrome_min.y + 12.0f));

    // Mode Selector Ghost Button
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 4.0f);
    ImGui::PushStyleColor(ImGuiCol_Button,
                          TokenColorF(UITokens::BackgroundDark, 0u));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                          TokenColorF(UITokens::TextSecondary, 35u));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
    ...
    if (last_status_text != viewport_status) {
      last_status_text = viewport_status;
      status_reveal_t = 0.0f;
    }
    status_reveal_t = std::min(1.0f, status_reveal_t + dt * 2.5f);

    std::string revealed_status = viewport_status;
    if (status_reveal_t < 1.0f) {
      size_t visible_len =
          static_cast<size_t>(revealed_status.length() * status_reveal_t);
      revealed_status = revealed_status.substr(0, visible_len) + "_";
    }
    ...
    if (!gs.tool_runtime.viewport_scene_stats_collapsed) {
      ...
      const auto deduped_at =
          [&](RogueCity::Core::Editor::ViewportRenderLayer layer) {
            return viewport_render.deduped_handles[static_cast<size_t>(layer)];
          };
      const uint32_t deduped_total =
          deduped_at(
      ...
      const char *frame_correlation_label = "Stable O(visible)";
      if (viewport_render.fallback_full_scan_count > 0u) {
        frame_correlation_label = "Fallback Scan Path";
      } else if (frame_ms > 16.0f &&
                 viewport_render.visible_cell_count > 512u) {
        frame_correlation_label = "Cell Coverage Bound";
      } else if (frame_ms > 16.0f && deduped_total > 2500u) {
        frame_correlation_label = "Visible Geometry Bound";
      } else if (frame_ms > 16.0f) {
        frame_correlation_label = "UI/GPU Bound";
      }

      ImGui::TextColored(TokenColorF(UITokens::InfoBlue, 255u),
                         "Viewport Render Attribution");
      ImGui::TextColored(
          TokenColorF(UITokens::TextSecondary, 230u),
          "Cells: %u | Fallback: %u | %s", viewport_render.visible_cell_count,
      ...
      draw_list->AddLine(ImVec2(uni_chrome_min.x + 12.0f,
                                ImGui::GetCursorScreenPos().y + 8.0f),
                         ImVec2(uni_chrome_min.x + uni_chrome_w - 12.0f,
                                ImGui::GetCursorScreenPos().y + 8.0f),
                         TokenColor(UITokens::TextSecondary, 100u), 1.0f);
      ImGui::SetCursorScreenPos(ImVec2(ImGui::GetCursorScreenPos().x,
                                       ImGui::GetCursorScreenPos().y + 16.0f));

      ImGui::TextColored(TokenColorF(UITokens::TextSecondary, 230u),
                         "Axioms: %zu | Roads: %zu | Districts: %zu",
    ...
    // Compass
    const ImVec2 compass_center(uni_chrome_min.x + uni_chrome_w - 24.0f,
  ...
  gs.validation_overlay.errors =
      RogueCity::Core::Validation::CollectOverlayValidationErrors(gs);

  auto draw_polyline_highlight =
      [&](const std::vector<RogueCity::Core::Vec2> &points, ImU32 color,
          float thickness, bool closed) {
        if (points.size() < 2) {
          return;
        }
        s_overlay_scratch_screen_points.clear();
        s_overlay_scratch_screen_points.reserve(points.size());
        for (const auto &point : points) {
          s_overlay_scratch_screen_points.push_back(
              s_primary_viewport->world_to_screen(point));
        }
        draw_list->AddPolyline(
            s_overlay_scratch_screen_points.data(),
            static_cast<int>(s_overlay_scratch_screen_points.size()), color,
            closed, thickness);
      };
  ...
  if (current_state == RogueCity::Core::Editor::EditorState::Editing_Roads &&
      gs.tool_runtime.road_spline_subtool ==
          RogueCity::Core::Editor::RoadSplineSubtool::Selection) {
    if (gs.hovered_entity.has_value() &&
        gs.hovered_entity->kind ==
            RogueCity::Core::Editor::VpEntityKind::Road) {
      if (RogueCity::Core::Road *hovered =
              FindRoadMutable(gs, gs.hovered_entity->id);
          hovered != nullptr) {
        draw_polyline_highlight(hovered->points,
                                TokenColor(UITokens::YellowWarning, 220u), 3.2f,
                                false);
      }
    }
    if (gs.selection.selected_road) {
      if (RogueCity::Core::Road *selected =
              FindRoadMutable(gs, gs.selection.selected_road->id);
          selected != nullptr) {
        draw_polyline_highlight(selected->points,
                                TokenColor(UITokens::CyanAccent, 220u), 2.6f,
                                false);
      }
    }
  }
  if (current_state == RogueCity::Core::Editor::EditorState::Editing_Water &&
      gs.tool_runtime.water_spline_subtool ==
          RogueCity::Core::Editor::WaterSplineSubtool::Selection) {
    if (gs.hovered_entity.has_value() &&
        gs.hovered_entity->kind ==
            RogueCity::Core::Editor::VpEntityKind::Water) {
      if (RogueCity::Core::WaterBody *hovered =
              FindWaterMutable(gs, gs.hovered_entity->id);
          hovered != nullptr) {
        const bool closed = hovered->type != RogueCity::Core::WaterType::River;
        draw_polyline_highlight(hovered->boundary,
                                TokenColor(UITokens::YellowWarning, 220u), 3.0f,
                                closed);
      }
    }
    const auto *primary = gs.selection_manager.Primary();
    if (primary != nullptr &&
        primary->kind == RogueCity::Core::Editor::VpEntityKind::Water) {
      if (RogueCity::Core::WaterBody *selected =
              FindWaterMutable(gs, primary->id);
          selected != nullptr) {
        const bool closed = selected->type != RogueCity::Core::WaterType::River;
        draw_polyline_highlight(selected->boundary,
                                TokenColor(UITokens::InfoBlue, 220u), 2.4f,
                                closed);
      }
    }
  }
  ...
  if (current_state == RogueCity::Core::Editor::EditorState::Editing_Roads &&
    ...
    if (RogueCity::Core::Road *road = FindRoadMutable(gs, road_id);
        road != nullptr) {
      const float world_to_screen =
          std::max(0.05f, s_primary_viewport->world_to_screen_scale(1.0f));
      const double handle_world_length =
          std::max(4.0, 20.0 / static_cast<double>(world_to_screen));
      for (size_t i = 0; i < road->points.size(); ++i) {
        const ImVec2 p = s_primary_viewport->world_to_screen(road->points[i]);
        const ImU32 color =
            (s_non_axiom_interaction.road_vertex_drag.active &&
        ...
        draw_list->AddCircle(
            p, 6.0f, TokenColor(UITokens::BackgroundDark, 200u), 12, 1.0f);

        if (gs.tool_runtime.road_spline_subtool ==
                RogueCity::Core::Editor::RoadSplineSubtool::HandleTangents &&
            i > 0 && i + 1 < road->points.size()) {
          ...
          if (in_dir.lengthSquared() > 1e-8) {
            in_dir.normalize();
            const RogueCity::Core::Vec2 in_handle_world =
                anchor + in_dir * handle_world_length;
            const ImVec2 in_handle =
                s_primary_viewport->world_to_screen(in_handle_world);
            const bool in_active =
                s_non_axiom_interaction.road_vertex_drag.active &&
                s_non_axiom_interaction.road_vertex_drag.vertex_index == i &&
                s_non_axiom_interaction.road_vertex_drag
                    .tangent_handle_active &&
                !s_non_axiom_interaction.road_vertex_drag.tangent_outgoing;
            draw_list->AddLine(p, in_handle,
                               TokenColor(UITokens::AmberGlow, 190u), 1.4f);
            draw_list->AddCircleFilled(
                in_handle, 3.4f,
                in_active ? TokenColor(UITokens::YellowWarning)
                          : TokenColor(UITokens::AmberGlow, 225u),
                10);
          }
          if (out_dir.lengthSquared() > 1e-8) {
            out_dir.normalize();
            const RogueCity::Core::Vec2 out_handle_world =
                anchor + out_dir * handle_world_length;
            const ImVec2 out_handle =
                s_primary_viewport->world_to_screen(out_handle_world);
            const bool out_active =
                s_non_axiom_interaction.road_vertex_drag.active &&
                s_non_axiom_interaction.road_vertex_drag.vertex_index == i &&
                s_non_axiom_interaction.road_vertex_drag
                    .tangent_handle_active &&
                s_non_axiom_interaction.road_vertex_drag.tangent_outgoing;
            draw_list->AddLine(p, out_handle,
                               TokenColor(UITokens::AmberGlow, 190u), 1.4f);
            draw_list->AddCircleFilled(
                out_handle, 3.4f,
                out_active ? TokenColor(UITokens::YellowWarning)
                           : TokenColor(UITokens::AmberGlow, 225u),
                10);
          }
        }
      ...
      if (gs.spline_editor.enabled && road->points.size() >= 3) {
        ...
        const auto smooth_points =
            RogueCity::App::EditorManipulation::BuildCatmullRomSpline(
                road->points, options);
        if (smooth_points.size() >= 2) {
          s_overlay_scratch_screen_points.clear();
          s_overlay_scratch_screen_points.reserve(smooth_points.size());
          ...
          draw_list->AddPolyline(
              ...
              TokenColor(UITokens::AmberGlow, 185u), gs.spline_editor.closed,
              2.0f);
        }
      }
    }
  ...
  if (current_state ==
      ...
      (gs.district_boundary_editor.enabled ||
       gs.tool_runtime.district_subtool ==
           RogueCity::Core::Editor::DistrictSubtool::Select ||
       gs.tool_runtime.district_subtool ==
           RogueCity::Core::Editor::DistrictSubtool::Paint ||
       gs.tool_runtime.district_subtool ==
           RogueCity::Core::Editor::DistrictSubtool::Zone ||
       gs.tool_runtime.district_subtool ==
           RogueCity::Core::Editor::DistrictSubtool::Split ||
       gs.tool_runtime.district_subtool ==
           RogueCity::Core::Editor::DistrictSubtool::Merge)) {
    const uint32_t district_id = gs.selection.selected_district->id;
    if (RogueCity::Core::District *district =
            FindDistrictMutable(gs, district_id);
        district != nullptr) {
      for (size_t i = 0; i < district->border.size(); ++i) {
        const ImVec2 p =
            s_primary_viewport->world_to_screen(district->border[i]);
        const ImU32 color =
            (s_non_axiom_interaction.district_boundary_drag.active &&
        ...
        draw_list->AddRectFilled(ImVec2(p.x - 4.0f, p.y - 4.0f),
                                 ImVec2(p.x + 4.0f, p.y + 4.0f), color);
        draw_list->AddRect(
            ImVec2(p.x - 6.0f, p.y - 6.0f), ImVec2(p.x + 6.0f, p.y + 6.0f),
            TokenColor(UITokens::BackgroundDark, 190u), 0.0f, 0, 1.0f);
      }
    }
  }
    ...
    if (has_selected_water) {
      if (RogueCity::Core::WaterBody *water = FindWaterMutable(gs, primary->id);
        ...
        for (size_t i = 0; i < water->boundary.size(); ++i) {
          const ImVec2 p =
              s_primary_viewport->world_to_screen(water->boundary[i]);
          const ImU32 color =
              (s_non_axiom_interaction.water_vertex_drag.active &&
               s_non_axiom_interaction.water_vertex_drag.vertex_index == i)
                  ? TokenColor(UITokens::YellowWarning)
                  : TokenColor(UITokens::InfoBlue, 220u);
          draw_list->AddCircleFilled(p, 4.0f, color, 12);
          draw_list->AddCircle(
              p, 6.0f, TokenColor(UITokens::BackgroundDark, 180u), 12, 1.0f);

          if (gs.tool_runtime.water_spline_subtool ==
                  RogueCity::Core::Editor::WaterSplineSubtool::HandleTangents &&
              i > 0 && i + 1 < water->boundary.size()) {
            const RogueCity::Core::Vec2 anchor = water->boundary[i];
            RogueCity::Core::Vec2 in_dir =
                water->boundary[i] - water->boundary[i - 1];
            RogueCity::Core::Vec2 out_dir =
                water->boundary[i] - water->boundary[i + 1];
            if (in_dir.lengthSquared() > 1e-8) {
              in_dir.normalize();
              const RogueCity::Core::Vec2 in_handle_world =
              ...
              const bool in_active =
                  s_non_axiom_interaction.water_vertex_drag.active &&
                  s_non_axiom_interaction.water_vertex_drag.vertex_index == i &&
                  s_non_axiom_interaction.water_vertex_drag
                      .tangent_handle_active &&
                  !s_non_axiom_interaction.water_vertex_drag.tangent_outgoing;
              draw_list->AddLine(p, in_handle,
                                 TokenColor(UITokens::InfoBlue, 195u), 1.3f);
              draw_list->AddCircleFilled(
                  in_handle, 3.4f,
                  in_active ? TokenColor(UITokens::YellowWarning)
                            : TokenColor(UITokens::InfoBlue, 230u),
                  10);
            }
            if (out_dir.lengthSquared() > 1e-8) {
              out_dir.normalize();
  ...
  overlay_config.show_lot_boundaries =
      gs.lots.size() != 0 && gs.show_layer_lots; // Enable lot boundaries
  ...
  search_overlay.Render(gs, transform);
  ...
  if (s_non_axiom_interaction.selection_drag.lasso_active &&
    ...
    for (const auto &p : s_non_axiom_interaction.selection_drag.lasso_points) {
      s_overlay_scratch_screen_points.push_back(
          s_primary_viewport->world_to_screen(p));
    }
    draw_list->AddPolyline(
        s_overlay_scratch_screen_points.data(),
        static_cast<int>(s_overlay_scratch_screen_points.size()),
  ...
  const bool inspect_mode =
      ...
      ((gs.tool_runtime.active_domain ==
            RogueCity::Core::Editor::ToolDomain::Road &&
        gs.tool_runtime.road_subtool ==
            RogueCity::Core::Editor::RoadSubtool::Inspect) ||
       ((gs.tool_runtime.active_domain ==
             RogueCity::Core::Editor::ToolDomain::District ||
         gs.tool_runtime.active_domain ==
             RogueCity::Core::Editor::ToolDomain::Zone) &&
        gs.tool_runtime.district_subtool ==
            RogueCity::Core::Editor::DistrictSubtool::Inspect) ||
       (gs.tool_runtime.active_domain ==
            RogueCity::Core::Editor::ToolDomain::Lot &&
        gs.tool_runtime.lot_subtool ==
            RogueCity::Core::Editor::LotSubtool::Inspect) ||
       (gs.tool_runtime.active_domain ==
            RogueCity::Core::Editor::ToolDomain::Building &&
        gs.tool_runtime.building_subtool ==
            RogueCity::Core::Editor::BuildingSubtool::Inspect) ||
       (gs.tool_runtime.active_domain ==
            RogueCity::Core::Editor::ToolDomain::Water &&
        gs.tool_runtime.water_subtool ==
            RogueCity::Core::Editor::WaterSubtool::Inspect));
  if (inspect_mode && gs.hovered_entity.has_value()) {
    if (ImGui::BeginTooltip()) {
      const auto hovered = *gs.hovered_entity;
      const RogueCity::Core::Vec2 inspect_world =
          non_axiom_interaction.has_world_pos
              ? non_axiom_interaction.world_pos
              : s_primary_viewport->screen_to_world(mouse_pos);
      const auto nearest_vertex_index =
          [&](const std::vector<RogueCity::Core::Vec2> &points) -> int {
        if (points.empty()) {
          return -1;
        }
      ...
      case RogueCity::Core::Editor::VpEntityKind::Road:
        if (RogueCity::Core::Road *road = FindRoadMutable(gs, hovered.id);
            road != nullptr) {
      ...
      case RogueCity::Core::Editor::VpEntityKind::District:
        if (RogueCity::Core::District *district =
                FindDistrictMutable(gs, hovered.id);
            district != nullptr) {
          ImGui::Text("Vertices: %zu", district->border.size());
          ImGui::Text("Type: %u", static_cast<unsigned>(district->type));
          ImGui::Text("Nearest Vertex: %d",
                      nearest_vertex_index(district->border));
        }
        break;
      case RogueCity::Core::Editor::VpEntityKind::Lot:
        for (const auto &lot : gs.lots) {
          if (lot.id == hovered.id) {
            ...
            ImGui::Text("Nearest Vertex: %d",
                        nearest_vertex_index(lot.boundary));
            break;
          }
        }
        break;
      ...
      case RogueCity::Core::Editor::VpEntityKind::Water:
        if (RogueCity::Core::WaterBody *water =
                FindWaterMutable(gs, hovered.id);
            water != nullptr) {
          ...
          ImGui::Text("Nearest Vertex: %d",
                      nearest_vertex_index(water->boundary));
        }
        break;
      default:
        break;
  ...
  const bool viewport_window_hovered =
      ImGui::IsWindowHovered(ImGuiHoveredFlags_ChildWindows);

  if (viewport_window_hovered && s_minimap_visible &&
      !input_gate.imgui_wants_mouse) {
    const ImVec2 minimap_interact_pos = ImVec2(
        viewport_pos.x + viewport_size.x - kMinimapSize - kMinimapPadding,
        viewport_pos.y + kMinimapPadding);
    const ImVec2 minimap_interact_max =
        ImVec2(minimap_interact_pos.x + kMinimapSize,
               minimap_interact_pos.y + kMinimapSize);

    // Check if mouse is over minimap
    const ImVec2 mouse_screen = ImGui::GetMousePos();
    const bool minimap_overlay_hovered =
        (mouse_screen.x >= minimap_interact_pos.x &&
         mouse_screen.x <= minimap_interact_max.x &&
         mouse_screen.y >= minimap_interact_pos.y &&
         mouse_screen.y <= minimap_interact_max.y);
    ...
    if (minimap_overlay_hovered) {
      ...
      if (allow_viewport_key_actions && ImGui::IsKeyPressed(ImGuiKey_L)) {
        if (io.KeyShift) {
          ...
          // First press enters manual mode and pins the current effective
          // level.
          s_minimap_auto_lod = false;
          s_minimap_lod = s_minimap_effective_lod;
        } else {
  ...
  // Validation errors are shown in the Tools strip (and can still be overlayed
  // here if desired).
  draw_list->PopClipRect();

  const ToolLibrary preferred_library =
      RC_UI::Commands::CommandLibraryForDomain(gs.tool_runtime.active_domain);
  RC_UI::Commands::DrawSmartMenu(s_smart_command_menu, command_dispatch);
  RC_UI::Commands::DrawPieMenu(s_pie_command_menu, preferred_library,
                               command_dispatch);
...
void Draw(float dt) {
  ...
  uiint.BeginPanel(
      ...
      true);
  uiint.RegisterWidget(
      {"viewport", "Primary Viewport", "viewport.primary", {"viewport"}});
