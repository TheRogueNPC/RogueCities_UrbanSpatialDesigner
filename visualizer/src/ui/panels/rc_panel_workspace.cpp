...
static constexpr const char *kDefaultMockupPath =
...
void ForceMockupReload() {
  ...
  std::snprintf(s_mockup_status, sizeof(s_mockup_status),
                ...
                result->layout.tool_deck_ratio, kDefaultMockupPath);
}
...
void DrawContent(float dt) {
  ...
  uiint.BeginPanel({"WorkspaceSelector",
                    "Workspace",
                    "settings",
                    "Left",
  ...
  if (ImGui::Button("Save", ImVec2(48.0f, 0.0f))) {
    ...
    if (SaveWorkspacePreset(s_preset_name, &err)) {
      ...
      std::snprintf(s_last_error, sizeof(s_last_error), "Save failed: %s",
                    err.c_str());
      s_last_status[0] = '\0';
    }
    uiint.RegisterWidget(
        {"button", "Save preset", "workspace.save_preset", {"preset"}});
  }
  ...
  Panels::UiSettings::DrawContent(dt);
...
void Draw(float dt) {
  ...
  if (!RC_UI::BeginDockableWindow(
          "Workspace##WorkspacePanel", s_workspace_window, "Left",
