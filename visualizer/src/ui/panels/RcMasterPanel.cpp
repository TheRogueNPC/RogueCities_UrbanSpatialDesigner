...
void RcMasterPanel::DrawActiveDrawer(DrawContext& ctx) {
    ...
    ImGui::BeginChild("##DrawerContent", ImVec2(0, 0), false, ImGuiWindowFlags_None);
...
void RcMasterPanel::DrawSearchOverlay(DrawContext& ctx) {
    ...
    if (ImGui::BeginPopupModal("##PanelSearch", &m_search_open, 
        ...
        bool enter_pressed = ImGui::InputText("##search", m_search_filter, sizeof(m_search_filter),
            ...
            for (size_t i = 0; i < m_search_results.size(); ++i) {
                ...
                if (is_selected) {
                    if (ImGui::IsKeyPressed(ImGuiKey_DownArrow)) {
                        m_search_selected_index = std::min(m_search_selected_index + 1, 
...
std::vector<PanelType> RcMasterPanel::FilterPanelsBySearch(const std::string& query) {
    auto& registry = PanelRegistry::Instance();
...
void RcMasterPanel::CloseSearch() {
    m_search_open = false;
}
