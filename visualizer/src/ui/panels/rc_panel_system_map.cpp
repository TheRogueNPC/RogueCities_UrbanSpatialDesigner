...
void DrawContent(float dt) {
  ...
  const ImVec2 center(panel_pos.x + panel_size.x * 0.5f,
                      panel_pos.y + panel_size.y * 0.5f);
  const float radius =
      (panel_size.x < panel_size.y ? panel_size.x : panel_size.y) * 0.20f;

  static ReactiveF focus;
  focus.target = ImGui::IsWindowHovered() ? 1.0f : 0.0f;
  focus.Update(dt);

  // Consume the full content region so ImGui knows our bounds before we
  // manually draw
  ImGui::Dummy(panel_size);

  draw_list->AddRectFilled(
      panel_pos, ImVec2(panel_pos.x + panel_size.x, panel_pos.y + panel_size.y),
      WithAlpha(UITokens::BackgroundDark, 245), UITokens::RoundingPanel);
  draw_list->AddCircleFilled(center, radius,
                             WithAlpha(UITokens::PanelBackground, 210), 64);
  draw_list->AddCircle(center, radius + 18.0f * focus.v,
                       WithAlpha(UITokens::CyanAccent, 180), 64,
                       UITokens::BorderNormal);
  draw_list->AddCircle(center, radius + 36.0f * focus.v,
                       WithAlpha(UITokens::MagentaHighlight, 145), 64,
                       UITokens::BorderThin);
  Components::DrawScanlineBackdrop(
      panel_pos, ImVec2(panel_pos.x + panel_size.x, panel_pos.y + panel_size.y),
      static_cast<float>(ImGui::GetTime()), UITokens::CyanAccent);
  ...
  if (has_bounds) {
    const auto world_to_map = [&](const RogueCity::Core::Vec2 &p) {
      ...
      const float v =
          static_cast<float>((p.y - bounds.min.y) / bounds.height());
      return ImVec2(panel_pos.x + u * panel_size.x,
                    panel_pos.y + v * panel_size.y);
    };

    const auto map_to_world = [&](const ImVec2 &map_pos) {
      const double u = std::clamp(static_cast<double>(map_pos.x - panel_pos.x) /
                                      static_cast<double>(panel_size.x),
                                  0.0, 1.0);
      const double v = std::clamp(static_cast<double>(map_pos.y - panel_pos.y) /
                                      static_cast<double>(panel_size.y),
                                  0.0, 1.0);
      return RogueCity::Core::Vec2(bounds.min.x + u * bounds.width(),
    ...
    if (systems_map.show_districts) {
      for (const auto &district : gs.districts) {
        ...
        draw_list->AddPolyline(screen_points.data(),
                               ...
                               WithAlpha(UITokens::GreenHUD, 150), true, 1.0f);
      }
    }
    ...
    if (systems_map.show_lots) {
      for (const auto &lot : gs.lots) {
        ...
        draw_list->AddPolyline(screen_points.data(),
                               static_cast<int>(screen_points.size()),
                               WithAlpha(UITokens::AmberGlow, 120), true, 0.9f);
      }
    }
    ...
    if (systems_map.show_roads) {
      for (const auto &road : gs.roads) {
        ...
        for (size_t i = 1; i < road.points.size(); ++i) {
          draw_list->AddLine(world_to_map(road.points[i - 1]),
                             world_to_map(road.points[i]),
                             WithAlpha(UITokens::CyanAccent, 180), 1.2f);
        }
      }
    }
    ...
    if (systems_map.show_water) {
      for (const auto &water : gs.waterbodies) {
        ...
        for (size_t i = 1; i < water.boundary.size(); ++i) {
          draw_list->AddLine(world_to_map(water.boundary[i - 1]),
                             world_to_map(water.boundary[i]),
                             WithAlpha(UITokens::InfoBlue, 170), 1.1f);
        }
      }
    }
    ...
    if (systems_map.show_buildings) {
      for (const auto &building : gs.buildings) {
        ...
        draw_list->AddCircleFilled(p, 2.0f,
                                   WithAlpha(UITokens::TextPrimary, 180), 8);
      }
    }
    ...
    if (systems_map.show_world_constraints && gs.world_constraints.isValid()) {
      draw_list->AddRect(
          ...
          WithAlpha(UITokens::MagentaHighlight, 120), 0.0f, 0, 1.0f);
    }
    ...
    if (mouse_in_map) {
      ...
      if (systems_map.enable_hover_query) {
        const float pick_radius = static_cast<float>(
            std::max(bounds.width(), bounds.height()) * 0.018);
        query_hit =
            QuerySystemsMapEntity(gs, systems_map, cursor_world, pick_radius);
        if (query_hit.valid) {
          ...
          systems_map.hovered_frame = gs.frame_counter;
          char label_buf[64]{};
          std::snprintf(label_buf, sizeof(label_buf), "%s #%u", query_hit.label,
                        query_hit.id);
          systems_map.hovered_label = label_buf;

          if (systems_map.enable_click_select &&
  ...
  ImGui::Dummy(ImVec2(panel_size.x - 24.0f, 0.0f)); // Register bounds with layout system
  ImGui::SetCursorScreenPos(ImVec2(panel_pos.x + 12.0f, panel_pos.y + 10.0f));
  ...
  if (query_hit.valid) {
    ImGui::Text("Query: %s #%u (d=%.2f)", SystemsMapKindLabel(query_hit.kind),
                query_hit.id, query_hit.distance);
  } else {
...
RC_PANEL_DRAW_IMPL(
    "System Map", "System Map", "nav", "Left",
