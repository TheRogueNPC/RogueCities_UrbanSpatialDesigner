...
void DrawContent(float dt) {
      ...
      "Theme Selection", UITokens::InfoBlue, panel_focused);
  {
    ...
    if (theme_names.empty()) {
      ...
      s_selected_theme_index = std::clamp(
          s_selected_theme_index, 0, static_cast<int>(theme_names.size()) - 1);
    }
    ...
    color_flags |= s_show_hex_by_default ? ImGuiColorEditFlags_DisplayHex
    ...
    ThemeColorRow rows[] = {
        ...
        {"Secondary", "theme_color_secondary",
         &active_theme_mut.secondary_accent},
    ...
    if (ImGui::BeginTable("theme_color_editor", two_columns ? 2 : 1,
      ...
      "Display Options", UITokens::InfoBlue, panel_focused);
  {
    ...
    uiint.RegisterWidget({"checkbox",
                          ...
                          "ui.dpi_scale_viewports_enabled",
                          {"ui", "settings", "dpi"}});
    ...
    if (ImGui::Combo("Viewport Right-Click Mode", &context_mode,
      ...
      context_mode = std::clamp(
          context_mode, 0, static_cast<int>(kCommandModeLabels.size()) - 1);
      gs.config.viewport_context_default_mode =
          static_cast<RogueCity::Core::Editor::ViewportCommandMode>(
              context_mode);
    }
    uiint.RegisterWidget({"combo",
                          "Viewport Right-Click Mode",
