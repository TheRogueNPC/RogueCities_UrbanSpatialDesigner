...
NonAxiomInteractionResult ProcessNonAxiomViewportInteractionPipeline(
            ...
            if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
                ...
                if (gs.gizmo.operation == GizmoOperation::Translate) {
                    ...
                    const RogueCity::Core::Vec2 prev =
                        interaction_state->gizmo_drag.previous_world - interaction_state->gizmo_drag.pivot;
                    ...
                    const double prev_dist = std::max(
                        1e-5,
                        interaction_state->gizmo_drag.previous_world.distanceTo(interaction_state->gizmo_drag.pivot));
    ...
    if (!consumed_interaction) {
        consumed_interaction = Handlers::HandleRoadVertexEdits(interaction_context, *interaction_state);
    }

    if (!consumed_interaction) {
        consumed_interaction = Handlers::HandleDistrictVertexEdits(interaction_context, *interaction_state);
    }
    ...
    if (!consumed_interaction && interaction_state->selection_drag.box_active) {
        ...
        if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
            auto region_items = Handlers::QueryRegionFromViewportIndex(
                gs,
                [&](const RogueCity::Core::Vec2& p) {
                    return Handlers::PointInPolygon(p, interaction_state->selection_drag.lasso_points);
                });
            gs.selection_manager.SetItems(std::move(region_items));
            RogueCity::Core::Editor::SyncPrimarySelectionFromManager(gs);
            gs.hovered_entity.reset();
    ...
    const bool left_click_unmodified = left_click_attempt && !io.KeyAlt && !io.KeyShift && !io.KeyCtrl;
...
std::optional<RogueCity::Core::Editor::SelectionItem> PickFromViewportIndexTestHook(
...
std::vector<RogueCity::Core::Editor::SelectionItem> QueryRegionFromViewportIndexTestHook(
    const RogueCity::Core::Editor::GlobalState& gs,
