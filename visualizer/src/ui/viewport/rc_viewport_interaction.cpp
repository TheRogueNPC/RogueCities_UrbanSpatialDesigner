...
const char *InteractionOutcomeString(InteractionOutcome outcome) {
  switch (outcome) {
  case InteractionOutcome::None:
    return "None";
  case InteractionOutcome::Mutation:
    return "Mutation";
  case InteractionOutcome::Selection:
    return "Selection";
  case InteractionOutcome::GizmoInteraction:
    return "GizmoInteraction";
  case InteractionOutcome::ActivateOnly:
    return "ActivateOnly";
  case InteractionOutcome::BlockedByInputGate:
    return "BlockedByInputGate";
  case InteractionOutcome::NoEligibleTarget:
...
ApplyViewportClampWithSpring(const RogueCity::Core::Vec2 &desired,
  ...
  if (TryCityBoundaryBounds(gs, min_bound, max_bound)) {
    return ClampToBoundsWithSpring(desired, min_bound, max_bound,
  ...
  return ClampToWorldConstraintsWithSpring(
      desired, gs.world_constraints, static_cast<double>(dt),
      gs.tool_runtime.viewport_clamp_overscroll,
      gs.tool_runtime.viewport_clamp_velocity,
      gs.tool_runtime.viewport_clamp_active);
}
...
void RequestDefaultContextCommandMenu(
    ...
    const ImVec2 &screen_pos, const CommandMenuStateBundle &state_bundle,
...
double DistanceToSegment(const RogueCity::Core::Vec2 &p,
...
bool PointInPolygon(const RogueCity::Core::Vec2 &point,
  ...
  return boost::geometry::covered_by(BoostPoint(point.x, point.y),
                                     boost_polygon);
}
...
bool IsSelectableKind(RogueCity::Core::Editor::VpEntityKind kind) {
  ...
  return kind == VpEntityKind::Road || kind == VpEntityKind::District ||
         kind == VpEntityKind::Lot || kind == VpEntityKind::Building ||
...
bool ProbeContainsPoint(const RogueCity::Core::Editor::GlobalState &gs,
                        RogueCity::Core::Editor::VpEntityKind kind, uint32_t id,
                        const RogueCity::Core::Vec2 &world_pos,
                        double world_radius, int &out_priority,
                        double &out_distance) {
  using RogueCity::Core::Editor::VpEntityKind;
    ...
    for (const auto &road : gs.roads) {
      ...
      for (size_t i = 1; i < road.points.size(); ++i) {
        min_distance = std::min(
            min_distance,
            DistanceToSegment(world_pos, road.points[i - 1], road.points[i]));
      }
  ...
  for (const auto &water : gs.waterbodies) {
    ...
    for (size_t i = 1; i < water.boundary.size(); ++i) {
      min_distance = std::min(
          min_distance, DistanceToSegment(world_pos, water.boundary[i - 1],
                                          water.boundary[i]));
    }
    if (water.boundary.size() >= 3 &&
        PointInPolygon(world_pos, water.boundary)) {
      out_priority = 4;
      out_distance = 0.0;
      return true;
    }
...
std::optional<RogueCity::Core::Editor::SelectionItem> PickFromViewportIndex(
  ...
  for (const auto &probe : gs.viewport_index) {
    ...
    int priority = 0;
    double distance = 0.0;
    if (!ProbeContainsPoint(gs, probe.kind, probe.id, world_pos, world_radius,
                            priority, distance)) {
      continue;
    }
...
QueryRegionFromViewportIndex(
  ...
  for (const auto &probe : gs.viewport_index) {
    ...
    const uint64_t key =
        (static_cast<uint64_t>(static_cast<uint8_t>(probe.kind)) << 32) |
        probe.id;
...
std::vector<RogueCity::Core::Vec2>
MakeSquareBoundary(const RogueCity::Core::Vec2 &center, double half_extent) {
  return {{center.x - half_extent, center.y - half_extent},
...
bool HandleDomainPlacementActions(
  ...
  if (editor_state == EditorState::Editing_Water) {
    ...
    uint32_t selected_water_id =
        (primary != nullptr && primary->kind == VpEntityKind::Water)
            ? primary->id
            : 0u;
    WaterBody *selected_water = selected_water_id != 0u
                                    ? FindWaterMutable(gs, selected_water_id)
                                    : nullptr;
    ...
    if (selected_water != nullptr && !selected_water->boundary.empty()) {
      if (gs.tool_runtime.water_spline_subtool ==
          ...
          !io.KeyShift && selected_water->boundary.size() >= 3) {
        ...
        if (best <= pick_radius) {
          const size_t prev_idx = (best_idx == 0)
                                      ? selected_water->boundary.size() - 1
                                      : best_idx - 1;
          const size_t next_idx =
              (best_idx + 1) % selected_water->boundary.size();
          selected_water->boundary[best_idx] =
              (selected_water->boundary[prev_idx] +
               selected_water->boundary[next_idx]) *
              0.5;
          MarkDirtyForSelectionKind(gs, VpEntityKind::Water);
          return true;
        }
      ...
      if (gs.tool_runtime.water_spline_subtool ==
        ...
        if (io.KeyCtrl && selected_water->boundary.size() > 2) {
          ...
          for (size_t i = 0; i + 1 < selected_water->boundary.size(); ++i) {
            const double d =
                DistanceToSegment(world_pos, selected_water->boundary[i],
          ...
          if (best_edge <=
              pick_radius * geometry_policy.edge_insert_multiplier) {
            selected_water->boundary.insert(
                selected_water->boundary.begin() +
                    static_cast<std::ptrdiff_t>(edge_idx + 1),
                world_pos);
            MarkDirtyForSelectionKind(gs, VpEntityKind::Water);
            return true;
          }
        ...
        if (io.KeyCtrl) {
          ...
          if (best_edge <=
            ...
            const RogueCity::Core::Vec2 midpoint =
                (selected_water->boundary[edge_idx] +
                 selected_water->boundary[edge_idx + 1]) *
                0.5;
            selected_water->boundary.insert(
                selected_water->boundary.begin() +
                    static_cast<std::ptrdiff_t>(edge_idx + 1),
      ...
      if (interaction_state.water_vertex_drag.active &&
        ...
        if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
          if (interaction_state.water_vertex_drag.vertex_index <
              selected_water->boundary.size()) {
            RogueCity::Core::Vec2 snapped = world_pos;
            SnapToGrid(snapped, gs.district_boundary_editor.snap_to_grid,
                       gs.district_boundary_editor.snap_size);

            const size_t anchor_index =
                interaction_state.water_vertex_drag.vertex_index;
            const RogueCity::Core::Vec2 anchor_before =
                selected_water->boundary[anchor_index];
            if (gs.tool_runtime.water_spline_subtool ==
                WaterSplineSubtool::SnapAlign) {
            ...
            if (use_falloff &&
                geometry_policy.water_falloff_radius_world > 0.0) {
              for (size_t i = 0; i < selected_water->boundary.size(); ++i) {
                const double distance =
                    selected_water->boundary[i].distanceTo(anchor_before);
                if (distance > geometry_policy.water_falloff_radius_world) {
                  continue;
                }
                double weight = RogueCity::App::Tools::ComputeFalloffWeight(
                    distance, geometry_policy.water_falloff_radius_world);
                if (gs.tool_runtime.water_subtool == WaterSubtool::Erode) {
                  weight *= 0.72;
                } else if (gs.tool_runtime.water_subtool ==
                           WaterSubtool::Flow) {
                  weight *= 0.9;
                }
  ...
  if (editor_state == EditorState::Editing_Roads && add_click &&
  ...
  if (editor_state == EditorState::Editing_Districts && add_click &&
    ...
    const auto target =
        PickFromViewportIndex(gs, world_pos, interaction_metrics);
    if (target.has_value() && target->kind == VpEntityKind::District &&
        target->id != primary_id) {
      RogueCity::Core::Vec2 primary_anchor{};
      RogueCity::Core::Vec2 target_anchor{};
      const bool anchors_ok =
          ResolveSelectionAnchor(gs, VpEntityKind::District, primary_id,
                                 primary_anchor) &&
          ResolveSelectionAnchor(gs, VpEntityKind::District, target->id,
                                 target_anchor);
      if (anchors_ok && primary_anchor.distanceTo(target_anchor) <=
                            geometry_policy.merge_radius_world) {
        if (RogueCity::Core::District *primary =
                FindDistrictMutable(gs, primary_id);
            primary != nullptr) {
          const uint32_t target_id = target->id;
          if (RogueCity::Core::District *other =
                  FindDistrictMutable(gs, target_id);
              other != nullptr && !other->border.empty()) {
            primary->border.insert(primary->border.end(), other->border.begin(),
                                   other->border.end());
  ...
  if (editor_state == EditorState::Editing_Lots && add_click &&
    ...
    lot.boundary = MakeSquareBoundary(
        world_pos, geometry_policy.lot_placement_half_extent);
    lot.area =
        static_cast<float>((geometry_policy.lot_placement_half_extent * 2.0) *
  ...
  if (editor_state == EditorState::Editing_Lots && add_click &&
    ...
    const auto target =
        PickFromViewportIndex(gs, world_pos, interaction_metrics);
    if (target.has_value() && target->kind == VpEntityKind::Lot &&
        target->id != primary_id) {
      RogueCity::Core::Vec2 primary_anchor{};
      RogueCity::Core::Vec2 target_anchor{};
      const bool anchors_ok =
          ResolveSelectionAnchor(gs, VpEntityKind::Lot, primary_id,
                                 primary_anchor) &&
          ResolveSelectionAnchor(gs, VpEntityKind::Lot, target->id,
                                 target_anchor);
      if (anchors_ok && primary_anchor.distanceTo(target_anchor) <=
                            geometry_policy.merge_radius_world) {
        const uint32_t target_id = target->id;
        for (size_t i = 0; i < gs.lots.size(); ++i) {
          ...
          for (size_t j = 0; j < gs.lots.size(); ++j) {
            ...
            h_primary->boundary.insert(h_primary->boundary.end(),
                                       h_other->boundary.begin(),
                                       h_other->boundary.end());
            h_primary->area += h_other->area;
            h_primary->centroid =
                (h_primary->centroid + h_other->centroid) * 0.5;
  ...
  if (editor_state == EditorState::Editing_Lots && add_click &&
    ...
    if (lot != nullptr) {
      ...
      const bool vertical_split =
          std::abs(world_pos.x - center.x) >= std::abs(world_pos.y - center.y);
  ...
  if (editor_state == EditorState::Editing_Lots && add_click &&
    ...
    LotToken *lot = FindLotMutable(gs, gs.selection.selected_lot->id);
    if (lot != nullptr) {
      RogueCity::Core::Vec2 center = lot->boundary.empty()
                                         ? lot->centroid
                                         : PolygonCentroid(lot->boundary);
      SnapToGrid(center, gs.district_boundary_editor.snap_to_grid,
                 gs.district_boundary_editor.snap_size);

      double half_extent = geometry_policy.lot_placement_half_extent;
      if (lot->area > 1.0f) {
        half_extent =
            std::max(6.0, std::sqrt(static_cast<double>(lot->area)) * 0.5);
      }
  ...
  if (editor_state == EditorState::Editing_Buildings && add_click &&
      gs.tool_runtime.building_subtool == BuildingSubtool::Place) {
    BuildingSite building{};
    building.id = NextBuildingId(gs);
    building.lot_id =
        gs.selection.selected_lot ? gs.selection.selected_lot->id : 0u;
    building.district_id = gs.selection.selected_district
                               ? gs.selection.selected_district->id
                               : 0u;
    building.position = world_pos;
    building.uniform_scale =
        static_cast<float>(geometry_policy.building_default_scale);
...
void ProcessViewportCommandTriggers(
  ...
  const bool allow_viewport_key_actions =
      RC_UI::AllowViewportKeyActions(params.input_gate);
  const ImGuiIO &io = ImGui::GetIO();
  ...
  const bool allow_command_hotkeys =
      allow_viewport_key_actions && params.in_viewport &&
      !params.minimap_hovered && !io.WantTextInput && !ImGui::IsAnyItemActive();
  ...
  if (params.editor_config->viewport_hotkey_domain_context_enabled) {
    ...
    for (size_t i = 0; i < kDomainKeys.size(); ++i) {
      ...
      if (!hold.opened && hold.held_seconds >= kHoldOpenSeconds) {
        RequestDefaultContextCommandMenu(*params.editor_config,
                                         params.mouse_pos, state_bundle,
...
AxiomInteractionResult
ProcessAxiomViewportInteraction(const AxiomInteractionParams &params) {
  AxiomInteractionResult result{};
  if (!params.axiom_mode || !params.allow_viewport_mouse_actions ||
      params.primary_viewport == nullptr || params.axiom_tool == nullptr) {
    return result;
  }
  ...
  const bool pan = ((!io.KeyAlt) && io.MouseDown[ImGuiMouseButton_Middle]) ||
  ...
  if (pan) {
    ...
    if (params.global_state != nullptr &&
      ...
      const RogueCity::Core::Vec2 clamped =
          ApplyViewportClampWithSpring(cam, *params.global_state, io.DeltaTime);
  ...
  if (!pan && params.global_state != nullptr &&
      params.global_state->config.feature_camera_bounce_clamp) {
    ...
    if (runtime.viewport_clamp_active ||
      ...
      const float z = params.primary_viewport->get_camera_z();
      const RogueCity::Core::Vec2 cam =
          params.primary_viewport->get_camera_xy();
      const RogueCity::Core::Vec2 settled =
          ApplyViewportClampWithSpring(cam, *params.global_state, io.DeltaTime);
...
NonAxiomInteractionResult ProcessNonAxiomViewportInteraction(
    ...
    NonAxiomInteractionState *interaction_state) {
  // Contract anchors: BuildToolInteractionMetrics(...),
  // ResolveToolGeometryPolicy(...), and requires_explicit_generation are
  // handled inside the delegated pipeline module.
