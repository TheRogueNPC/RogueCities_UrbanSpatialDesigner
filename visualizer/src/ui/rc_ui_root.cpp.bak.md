...
constexpr std::array kShortcutDefinitions = {
    ...
    ShortcutDefinition{{Keymap::Action::kRulerToggle,    "View", "Toggle Ruler"},
                       {ImGuiKey_L, false, false, false, false}},
    ShortcutDefinition{{Keymap::Action::kRulerCrosshair, "View", "Ruler: Crosshair Mode"},
                       {ImGuiKey_L, false, true,  false, false}},
    ShortcutDefinition{{Keymap::Action::kRulerDivMode,   "View", "Ruler: Div Snap Mode"},
    ...
    if (ImGui::BeginMenu("View")) {
      ...
      if (ImGui::BeginMenu("Ruler")) {
        ...
        ImGui::BeginDisabled(!ruler_ticks);
        {
          ...
          if (ImGui::MenuItem("Crosshair Mode",
                              Keymap::ShortcutLabel(
                                  Keymap::Action::kRulerCrosshair),
                              ch))
            Panels::AxiomEditor::SetRulerCrosshairMode(!ch);

          bool snap = Panels::AxiomEditor::IsRulerSnapEnabled();
          if (ImGui::MenuItem("Snap to Division",
                              Keymap::ShortcutLabel(
                                  Keymap::Action::kRulerDivMode),
                              snap))
      ...
      // ── Guides sub-group ─────────────────────────────────────────────
      // Manages all placed div lines (Photoshop-style guide lines).
      // Drag from ruler strips to place; or use right-click > Guides.
      if (ImGui::BeginMenu("Guides")) {
        auto &div = RC_UI::Viewport::GetViewportOverlays().div_lines;
        if (div.lines.empty()) {
          ImGui::TextDisabled("  (drag from ruler strip to add)");
        } else {
          // One row per div line: [checkbox] label  [X delete]
          for (int i = 0; i < static_cast<int>(div.lines.size()); ++i) {
            auto &d = div.lines[static_cast<std::size_t>(i)];
            ImGui::PushID(d.id);

            ImGui::Checkbox("##en", &d.enabled);
            ImGui::SameLine();
            ...
            if (ImGui::SmallButton("X")) {
              div.Remove(d.id);
              ImGui::PopID();
              break; // iterator invalidated; menu will refresh next frame
            }
          ...
          ImGui::Separator();
          if (ImGui::MenuItem("Clear All Guides"))
            div.Clear();
        }

        ImGui::Separator();
        if (ImGui::MenuItem("Save Guides..."))
          div.SaveToJson("div_guides.json");
        if (ImGui::MenuItem("Load Guides..."))
          div.LoadFromJson("div_guides.json");
...
static void DrawGlobalScanlines() {
  ...
  if (excl && excl->Active && !excl->Hidden) {
    ...
    const ImVec2 e_max = ImVec2(excl->Pos.x + excl->Size.x,
                                excl->Pos.y + excl->Size.y);
