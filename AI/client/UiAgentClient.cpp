...
std::vector<UiCommand> UiAgentClient::QueryAgent(
    ...
    // HTTP bridge is disabled at compile time.
    // Snapshot building and UiIntrospector remain fully operational for local
    // tooling; only the network round-trip is suppressed.
    (void)snapshot;
    (void)goal;
    return {};
#else
