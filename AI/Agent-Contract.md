...
## 4. UI Verification & Perception

If you modify the ImGui visualizer or UI components:
1.  **Do not guess if it works**. Run `rc-perceive-ui -Mode quick -Screenshot` via the dev shell.
2.  If you need to validate the full pipeline, run `rc-full-smoke -Port 7222 -Runs 1`.
3.  Ensure the three core layout sections (Viewport, Titlebar, Status) remain present.
...
## 6. Unified Collaboration Space

To prevent context fragmentation between GitHub Copilot, Gemini CLI, Claude, and local Gemma, you must use the **Shared Knowledge directory**: `AI/collaboration/`.
*   If you research a bug or plan an architecture, write your findings to a markdown file in `AI/collaboration/`.
*   If you are picking up a session, read the outstanding manifests in `AI/collaboration/` to absorb what the other AIs have accomplished.
*   **We are coworkers, not isolated volunteers.**
