// _USE_MATH_DEFINES must come before any <cmath> inclusion to expose M_PI on MSVC.
// Place it here so it wins even if a transitive header pulls in <cmath> first.
...
Road::Road(std::string id, double length, std::string junction,
           std::string name, bool left_hand_traffic)
    : length(length), id(id), junction(junction), name(name),
      left_hand_traffic(left_hand_traffic), ref_line(length) {}
...
double Road::get_lanesection_end(const LaneSection &lanesection) const {
  if (lanesection.road_id != this->id)
    log::error("LaneSection %f in Road #%s, not in Road #%s", lanesection.s0,
               lanesection.road_id.c_str(), this->id.c_str());
...
double Road::get_lanesection_end(const double lanesection_s0) const {
  ...
  return std::nextafter(
      next_s0,
      std::numeric_limits<double>::lowest()); // should be within lane section
}
...
Vec3D Road::get_xyz(const double s, const double t, const double h, Vec3D *_e_s,
                    Vec3D *_e_t, Vec3D *_e_h) const {
  // Hardening: Clamp s to [0, length] to prevent extrapolation beyond ref_line
  // bounds
  double s_clamped = std::max(0.0, std::min(s, this->length));

  const Vec3D s_vec = this->ref_line.derivative(s_clamped);
  const double theta = this->superelevation.evaluate(s_clamped);
  ...
  const Vec3D p0 = this->ref_line.get_xyz(s_clamped);
...
Vec3D Road::get_surface_pt(double s, const double t, Vec3D *vn) const {
  // Hardening: Handle empty roads or invalid lengths
  if (this->length <= 0)
    return {0, 0, 0};

  odr::check_and_repair(
      s >= 0, [&]() { s = 0; },
      "Road #%s: get_surface_pt for s < 0 invalid (s=%f), getting for s=0",
      this->id.c_str(), s);
  odr::check_and_repair(
      s <= this->length, [&]() { s = this->length; },
      "Road #%s: get_surface_pt for s > length invalid (s=%f, length=%f), "
      "getting for s=length",
      this->id.c_str(), s, this->length);

  const double lanesection_s0 = this->get_lanesection_s0(s);
  if (std::isnan(lanesection_s0)) {
    throw std::runtime_error(strfmt("cannot get road surface pt, no lane "
                                    "section for s %.3f, road length: %.3f",
                                    s, this->length));
  }

  const LaneSection &lanesection = this->s_to_lanesection.at(lanesection_s0);
  const Lane &lane = lanesection.get_lane(s, t);
  const Lane &inner_neighbor_lane =
      lanesection.get_lane(next_towards_zero(lane.id));
  ...
  if (lane.level) {
    ...
    const double superelev =
        this->superelevation.evaluate(s); // cancel out superelevation
...
std::set<double> Road::approximate_lane_border_linear(const Lane &lane,
  ...
  if (!outer) {
    const LaneSection &lanesection =
        this->s_to_lanesection.at(lane.key.lanesection_s0);
    const Lane &inner_neighbor_lane =
        lanesection.get_lane(next_towards_zero(lane.id));
...
Line3D Road::get_lane_border_line(const Lane &lane, const double s_start,
                                  const double s_end, const double eps,
                                  const bool outer) const {
  std::set<double> s_vals =
  ...
  for (const double &s : s_vals) {
    ...
    if (!outer)
      t = std::nextafter(
          t, lane.outer_border.evaluate(
                 s)); // ensure t is not on lane boundary but within lane
...
Line3D Road::get_lane_border_line(const Lane &lane, const double eps,
                                  const bool outer) const {
  const double s_end = this->get_lanesection_end(lane.key.lanesection_s0);
...
Mesh3D Road::get_lane_mesh(const Lane &lane, const double s_start,
                           const double s_end, const double eps,
  ...
  std::set<double> s_vals_inner_brdr =
      inner_neighbor_lane.outer_border.approximate_linear(eps, s_start, s_end);
  ...
  for (const double &s : s_vals) {
    ...
    const double t_inner_brdr = std::nextafter(
        inner_neighbor_lane.outer_border.evaluate(s),
        t_outer_brdr); // ensure t is not on lane boundary but within lane
    out_mesh.vertices.push_back(
...
Mesh3D Road::get_lane_mesh(const Lane &lane, const double eps,
                           std::vector<uint32_t> *outline_indices) const {
  const double s_end = this->get_lanesection_end(lane.key.lanesection_s0);
...
Mesh3D Road::get_roadmark_mesh(const Lane &lane, const RoadMark &roadmark,
  ...
  for (const double &s : s_vals) {
    ...
    const double t_edge_a = lane.outer_border.evaluate(s) +
                            roadmark.width * 0.5 + roadmark.t_offset;
...
Mesh3D Road::get_road_signal_mesh(const RoadSignal &road_signal) const {
  ...
  // 1. Inertial absolute positioning override
  if (road_signal.position_inertial.has_value()) {
    const double roll = road_signal.position_inertial->roll;
    const double pitch = road_signal.position_inertial->pitch;
    double hdg = road_signal.position_inertial->hdg;

    // orientation applies a 180 deg flip (ASAM §14.1)
    if (road_signal.orientation == "-") {
      hdg += M_PI;
    }

    const Mat3D rot_mat_inertial =
        EulerAnglesToMatrix<double>(roll, pitch, hdg);
    const Vec3D p0{road_signal.position_inertial->x,
                   road_signal.position_inertial->y,
                   road_signal.position_inertial->z};

    for (Vec3D &pt_uvz : road_signal_mesh.vertices) {
      pt_uvz = MatVecMultiplication(rot_mat_inertial, pt_uvz);
      pt_uvz = add(pt_uvz, p0);
      road_signal_mesh.st_coordinates.push_back(
          {road_signal.s0, road_signal.t0});
    }
    return road_signal_mesh;
  }

  // 2. Relative positioning (Road override or Base attributes)
  double s = road_signal.s0;
  double t = road_signal.t0;
  double z = road_signal.zOffset;
  double roll = road_signal.roll;
  double pitch = road_signal.pitch;
  double heading_offset = road_signal.hOffset;

  if (road_signal.position_road.has_value()) {
    s = road_signal.position_road->s;
    t = road_signal.position_road->t;
    z = road_signal.position_road->z_offset;
    roll = road_signal.position_road->roll;
    pitch = road_signal.position_road->pitch;
    heading_offset = road_signal.position_road->h_offset;
  }

  if (road_signal.orientation == "-") {
    heading_offset += M_PI; // ASAM 1.8 §14.1 defines "-" as rotated 180 degrees
  }

  const Mat3D rot_mat =
      EulerAnglesToMatrix<double>(roll, pitch, heading_offset);
  Vec3D e_s, e_t, e_h;
...
Mesh3D Road::get_road_object_mesh(const RoadObject &road_object,
  ...
  if (repeats_copy.empty() &&
      road_object.outlines.empty()) // handle single object as 1 object repeat
  {
  ...
  /* outline objects are not repeated, repeats only apply to the generic road
   * object (box or cylinder) */
  /* note: if road object has an outline object AND repeat this will create
   * generic objects at repeat AND the outline object (non-repeated) */
  ...
  for (const RoadObjectOutline &road_object_outline : road_object.outlines) {
    ...
    const bool is_flat_object = std::all_of(
        road_object_outline.outline.begin(), road_object_outline.outline.end(),
    ...
    if (!is_flat_object) {
      for (const RoadObjectCorner &corner : road_object_outline.outline) {
        ...
        if (corner.type == RoadObjectCorner::Type::Local_AbsZ ||
            corner.type == RoadObjectCorner::Type::Local_RelZ) {
    ...
    const std::vector<size_t> idx_patch_top =
        mapbox::earcut<size_t>(outline_road_obj_mesh.vertices.data(),
                               road_object_outline.outline.size());
