...
std::vector<LaneValidityRecord>
extract_lane_validity_records(const pugi::xml_node &xml_node) {
  std::vector<LaneValidityRecord> lane_validities;
  for (const auto &validity_node : xml_node.children("validity")) {
    LaneValidityRecord lane_validity{
        validity_node.attribute("fromLane").as_int(INT_MIN),
        validity_node.attribute("toLane").as_int(INT_MAX)};

    // fromLane should not be greater than toLane, since the standard defines
    // them as follows: fromLane - the minimum ID of lanes for which the object
    // is valid toLane - the maximum ID of lanes for which the object is valid
...
OpenDriveMap::OpenDriveMap(const std::string &xodr_file, const bool center_map,
  ...
  const pugi::xml_parse_result xml_parse_result =
      xml_doc.load_file(xodr_file.c_str());
  if (!xml_parse_result)
    log::error("Error parsing xml: %s", xml_parse_result.description());

  const pugi::xml_node odr_node = xml_doc.child("OpenDRIVE");

  if (const auto geoReference_node =
          odr_node.child("header").child("geoReference")) {
    this->proj4 = geoReference_node.text().as_string("");
    if (!this->proj4.empty())
      this->coordinate_system = CoordinateSystem::GlobalProjection;
  }

  if (const auto offset_node = odr_node.child("header").child("offset")) {
    this->x_offs = offset_node.attribute("x").as_double(0.0);
    this->y_offs = offset_node.attribute("y").as_double(0.0);
  }

  std::size_t cnt = 1;
  if (center_map) {
    for (const pugi::xml_node road_node : odr_node.children("road")) {
      for (const pugi::xml_node geometry_hdr_node :
  ...
  for (const pugi::xml_node junction_node : odr_node.children("junction")) {
    /* make junction — ASAM OpenDRIVE § 6.2 */
    const std::string junction_id = junction_node.attribute("id").as_string("");

    // § 6.2: type is "default" for physical junctions, "virtual" for
    // logical splits on long roads.  Default to "default" if absent.
    const std::string junction_type =
        junction_node.attribute("type").as_string("default");
    ...
    for (const pugi::xml_node connection_node :
      ...
      for (const pugi::xml_node lane_link_node :
           connection_node.children("laneLink")) {
        const JunctionLaneLink lane_link(
            lane_link_node.attribute("from").as_int(0),
    ...
    odr::check(num_conns > 0, "Junction #%s: 0 connections",
               junction_id.c_str());
    if (num_conns < 1)
      continue;

    for (const pugi::xml_node priority_node :
         junction_node.children("priority")) {
      const JunctionPriority junction_priority(
          priority_node.attribute("high").as_string(""),
          priority_node.attribute("low").as_string(""));
      junction.priorities.insert(junction_priority);
    }

    for (const pugi::xml_node controller_node :
    ...
    // -----------------------------------------------------------------------
    // § 6.3.3 / Code 24 — Pedestrian Cross-Paths
    // <crossPath id="..." crossingRoad="..." roadAtStart="..." roadAtEnd="...">
    //   <startLaneLink s="..." from="..." to="..."/>
    //   <endLaneLink   s="..." from="..." to="..."/>
    // </crossPath>
    // -----------------------------------------------------------------------
    for (const pugi::xml_node cp_node : junction_node.children("crossPath")) {
      const JunctionCrossPathLink start_link(
          cp_node.child("startLaneLink").attribute("s").as_double(0.0),
          cp_node.child("startLaneLink").attribute("from").as_int(0),
          cp_node.child("startLaneLink").attribute("to").as_int(0));

      const JunctionCrossPathLink end_link(
          cp_node.child("endLaneLink").attribute("s").as_double(0.0),
          cp_node.child("endLaneLink").attribute("from").as_int(0),
          cp_node.child("endLaneLink").attribute("to").as_int(0));

      junction.cross_paths.emplace_back(
          cp_node.attribute("id").as_string(""),
          cp_node.attribute("crossingRoad").as_string(""),
          cp_node.attribute("roadAtStart").as_string(""),
          cp_node.attribute("roadAtEnd").as_string(""), start_link, end_link);
    }

    // -----------------------------------------------------------------------
    // § 6.5 / Code 32 — Traffic Islands
    // <objects>
    //   <object type="trafficIsland" id="..." name="..." height="..."
    //           zOffset="...">
    //     <outlines>
    //       <outline id="..." fillType="...">
    ...
    // </objects>
    //
    // Note: Only objects whose type attribute equals "trafficIsland" are
    // captured here.  Other junction-level objects are currently ignored
    // (they may be added as a future extension).
    // -----------------------------------------------------------------------
    for (const pugi::xml_node obj_node :
         junction_node.child("objects").children("object")) {
      const std::string obj_type = obj_node.attribute("type").as_string("");
      if (obj_type != "trafficIsland")
        continue;

      JunctionTrafficIsland island(obj_node.attribute("id").as_string(""),
                                   obj_node.attribute("name").as_string(""),
                                   obj_node.attribute("height").as_double(0.0),
                                   obj_node.attribute("zOffset").as_double(0.0),
                                   ""); // fill_type resolved per outline below

      // Supports both v1.4 (<outline> directly under <object>) and
      // v1.45+ (<outlines> parent), mirroring road object parsing logic.
      const pugi::xml_node outlines_parent =
          obj_node.child("outlines") ? obj_node.child("outlines") : obj_node;

      for (const pugi::xml_node outline_node :
           outlines_parent.children("outline")) {
        // Capture fill_type from the first outline encountered.
        if (island.fill_type.empty())
          island.fill_type = outline_node.attribute("fillType").as_string("");

        for (const pugi::xml_node corner_node :
             outline_node.children("cornerLocal")) {
          island.outline.emplace_back(
              corner_node.attribute("u").as_double(0.0),
              corner_node.attribute("v").as_double(0.0),
              corner_node.attribute("z").as_double(0.0),
              corner_node.attribute("height").as_double(0.0));
        }
      }

      junction.traffic_islands.push_back(std::move(island));
    }
  }
  ...
  for (const pugi::xml_node road_node : odr_node.children("road")) {
    ...
    odr::check_and_repair(
        ...
        "Road #%s already exists, adding _dup suffix to id", road_id.c_str());
    ...
    odr::check_and_repair(
        road.length >= 0, [&]() { road.length = 0; },
        "Road #%s: length %f < 0, set length = 0", road_id.c_str(),
        road.length);
    ...
    for (const bool is_predecessor : {true, false}) {
      ...
      if (road_link_node) {
        ...
        odr::check_and_repair(
            ...
            road_id.c_str(), is_predecessor ? "predecessor" : "successor",
            type_str.c_str());
        link.type = (type_str == "road") ? RoadLink::Type::Road
                                         : RoadLink::Type::Junction;
        ...
        if (link.type == RoadLink::Type::Road) {
          ...
          link.contact_point = (contact_point_str == "start")
                                   ? RoadLink::ContactPoint::Start
                                   : RoadLink::ContactPoint::End;
        }
      }
    }

    /* parse road neighbors */
    for (const pugi::xml_node road_neighbor_node :
    ...
    for (const pugi::xml_node road_type_node : road_node.children("type")) {
      ...
      odr::check_and_repair(
          s >= 0, [&]() { s = 0; },
          "Road #%s: type record '%s' s %f < 0, set s=0", road_id.c_str(),
          type.c_str(), s);
    ...
    for (const pugi::xml_node geometry_hdr_node :
      ...
      odr::check_and_repair(
          s0 >= 0, [&]() { s0 = 0; },
          "Road #%s: planView::geometry::s %f < 0, set s=0", road_id.c_str(),
          s0);
      odr::check_and_repair(
          length >= 0, [&]() { length = 0; },
          "Road #%s: planView::geometry::length %f < 0, set length=0",
          road_id.c_str(), length);
      ...
      if (geometry_type == "line") {
        ...
        const double curv_start =
            geometry_node.attribute("curvStart").as_double(0.0);
        const double curv_end =
            geometry_node.attribute("curvEnd").as_double(0.0);
        if (!fix_spiral_edge_cases) {
          ...
          if (std::abs(curv_start) < 1e-6 && std::abs(curv_end) < 1e-6) {
            ...
            // In effect an arc
            road.ref_line.s0_to_geometry[s0] =
                std::make_unique<Arc>(s0, x0, y0, hdg0, length, curv_start);
          } else {
            // True spiral
            road.ref_line.s0_to_geometry[s0] = std::make_unique<Spiral>(
                s0, x0, y0, hdg0, length, curv_start, curv_end);
        ...
        const double curvature =
            geometry_node.attribute("curvature").as_double(0.0);
        road.ref_line.s0_to_geometry[s0] =
    ...
    for (auto &[xpath_query_str, cubic_profile] : cubic_profile_fields) {
      ...
      pugi::xpath_node_set nodes =
          road_node.select_nodes(xpath_query_str.c_str());
      for (pugi::xpath_node node : nodes) {
        ...
        odr::check_and_repair(
            s0 >= 0, [&]() { s0 = 0; }, "Road #%s: %s s %f < 0, set s=0",
            road_id.c_str(), xpath_query_str.c_str(), s0);
    ...
    if (with_lateral_profile) {
      for (pugi::xml_node crossfall_node :
        ...
        odr::check_and_repair(
            s0 >= 0, [&]() { s0 = 0; },
            "Road #%s: lateralProfile::crossfall::s %f < 0, set s=0",
            road_id.c_str(), s0);
    ...
    for (const pugi::xml_node lanesection_node :
      ...
      for (const pugi::xpath_node lane_xpath_node :
        ...
        odr::check(!lane_node.child("border"),
                   "Road #%s LaneSection %f Lane #%d: border definitions not "
                   "supported",
                   road_id.c_str(), s0, lane_id);
        ...
        if (const pugi::xml_attribute id_attr =
                lane_node.child("link").child("predecessor").attribute("id"))
          lane.predecessor = id_attr.as_int();
        if (const pugi::xml_attribute id_attr =
                lane_node.child("link").child("successor").attribute("id"))
          lane.successor = id_attr.as_int();

        for (const pugi::xml_node lane_width_node :
             lane_node.children("width")) {
          ...
          odr::check_and_repair(
              ...
              "Road #%s LaneSection %f Lane #%d: width::sOffset %f < 0, set "
              "sOffset=0",
              road_id.c_str(), s0, lane_id, s_offset);
          CubicPoly width_poly(a, b, c, d, s0 + s_offset);

          // OpenDRIVE Format Specification, Rev. 1.4, 3.3.1 General:
          // "The reference line itself is defined as lane zero and must not
          // have a width entry (i.e. its width must always be 0.0)."
          if (lane_id == 0)
            odr::check_and_repair(
                width_poly.is_zero(), [&]() { width_poly.set_zero(); },
        ...
        if (with_lane_height) {
          for (const pugi::xml_node lane_height_node :
               lane_node.children("height")) {
            double s_offset =
                lane_height_node.attribute("sOffset").as_double(0.0);
            const double inner =
                lane_height_node.attribute("inner").as_double(0.0);
            const double outer =
                lane_height_node.attribute("outer").as_double(0.0);

            odr::check_and_repair(
                s_offset >= 0, [&]() { s_offset = 0; },
                "Road #%s LaneSection %f Lane #%d: height::sOffset %f < 0, set "
                "sOffset=0",
                road_id.c_str(), s0, lane_id, s_offset);
        ...
        for (const pugi::xml_node roadmark_node :
             lane_node.children("roadMark")) {
          ...
          odr::check_and_repair(
              ...
              [&]() { roadmark_group.s_offset = 0; },
              "Road #%s LaneSection %f Lane #%d: roadMark::sOffset %f < 0, set "
              "sOffset=0",
              road_id.c_str(), s0, lane_id, roadmark_group.s_offset);
          const double roadmark_group_s0 = s0 + roadmark_group.s_offset;

          if (const pugi::xml_node roadmark_type_node =
                  roadmark_node.child("type")) {
            const std::string name =
                roadmark_type_node.attribute("name").as_string("");
            const double line_width_1 =
                roadmark_type_node.attribute("width").as_double(-1);

            for (const pugi::xml_node roadmarks_line_node :
              ...
              for (auto [val_name, val_ptr] :
                   {std::pair{"length", &(roadmarks_line.length)},
                    std::pair{"space", &(roadmarks_line.space)},
                    std::pair{"s_offset", &(roadmarks_line.s_offset)}}) {
                odr::check_and_repair(
                    (*val_ptr) >= 0, [p = val_ptr]() { *p = 0; },
                    "Road #%s LaneSection %f Lane #%d RoadMark %f: "
                    "type::line::%s %f < 0, set %s=0",
                    road_id.c_str(), s0, lane_id, roadmark_group_s0, val_name,
                    *val_ptr, val_name);
              }
      ...
      // relationship &*r == &*(i - 1) is always true" The reverse iterator
      // points to the element that is one before the element referred by the
      // id_lane_iter0!
      const std::map<int, Lane>::reverse_iterator r_id_lane_iter1(
          id_lane_iter0);
      for (auto r_iter = r_id_lane_iter1;
    ...
    if (with_road_objects) {
      const RoadObjectCorner::Type default_local_outline_type =
          abs_z_for_for_local_road_obj_outline
              ? RoadObjectCorner::Type::Local_AbsZ
              : RoadObjectCorner::Type::Local_RelZ;
      ...
      for (pugi::xml_node object_node :
        ...
        odr::check_and_repair(
            ...
            road_id.c_str(), road_object_id.c_str());

        const bool is_dynamic_object =
            std::string(object_node.attribute("dynamic").as_string("no")) ==
                    "yes"
                ? true
                : false;
        ...
        for (auto [val_name, val_ptr] :
          ...
          odr::check_and_repair((*val_ptr) >= 0, [p = val_ptr]() { *p = 0; },
                                "Road #%s: object::%s %f < 0, set %s=0",
                                road_id.c_str(), val_name, *val_ptr, val_name);
        }
        ...
        for (pugi::xml_node repeat_node : object_node.children("repeat")) {
          ...
          for (auto [val_name, val_ptr] :
               {std::pair{"s0", &(road_object_repeat.s0)},
            ...
            odr::check_and_repair(
                ...
                road_id.c_str(), road_object_id.c_str(), val_name, *val_ptr,
                val_name);
          }
          for (auto [val_name, val_ptr] :
               {std::pair{"length", &(road_object_repeat.length)},
                std::pair{"distance", &(road_object_repeat.distance)}}) {
            odr::check_and_repair(
                (*val_ptr) >= 0, [p = val_ptr]() { *p = 0; },
                "Road #%s Object #%s: repeat::%s %f < 0, set %s=0",
                road_id.c_str(), road_object_id.c_str(), val_name, *val_ptr,
        ...
        /* since v1.45 multiple <outline> are allowed and parent tag is
         * <outlines>, not <object>; this supports v1.4 and v1.45+ */
        const pugi::xml_node outlines_parent_node =
            object_node.child("outlines") ? object_node.child("outlines")
                                          : object_node;
        for (const pugi::xml_node outline_node :
             outlines_parent_node.children("outline")) {
          RoadObjectOutline road_object_outline(
              outline_node.attribute("id").as_int(-1),
          ...
          for (const pugi::xml_node corner_local_node :
               outline_node.children("cornerLocal")) {
            ...
            RoadObjectCorner road_object_corner_local(
                corner_local_node.attribute("id").as_int(-1), pt_local,
          ...
          for (const pugi::xml_node corner_road_node :
               outline_node.children("cornerRoad")) {
            ...
            RoadObjectCorner road_object_corner_road(
                corner_road_node.attribute("id").as_int(-1), pt_road,
        ...
        for (pugi::xml_node material_node : object_node.children("material")) {
          road_object.materials.emplace_back(
              material_node.attribute("friction").as_double(0.0),
              material_node.attribute("roughness").as_double(0.0),
              material_node.attribute("surface").as_string(""),
              material_node.attribute("roadMarkColor").as_string(""));
        }

        if (pugi::xml_node parking_node = object_node.child("parkingSpace")) {
          const std::string access_str =
              parking_node.attribute("access").as_string("all");
          RoadObjectParkingSpace::Access access =
              RoadObjectParkingSpace::Access::All;
          if (access_str == "bus")
            access = RoadObjectParkingSpace::Access::Bus;
          else if (access_str == "car")
            access = RoadObjectParkingSpace::Access::Car;
          else if (access_str == "electric")
            access = RoadObjectParkingSpace::Access::Electric;
          else if (access_str == "handicapped")
            access = RoadObjectParkingSpace::Access::Handicapped;
          else if (access_str == "residents")
            access = RoadObjectParkingSpace::Access::Residents;
          else if (access_str == "truck")
            access = RoadObjectParkingSpace::Access::Truck;
          else if (access_str == "women")
            access = RoadObjectParkingSpace::Access::Women;

          road_object.parking_spaces.emplace_back(
              access, parking_node.attribute("restrictions").as_string(""));
        }

        for (pugi::xml_node markings_node : object_node.children("markings")) {
          for (pugi::xml_node marking_node :
               markings_node.children("marking")) {
            const std::string side_str =
                marking_node.attribute("side").as_string("front");
            RoadObjectMarking::Side side = RoadObjectMarking::Side::Front;
            if (side_str == "left")
              side = RoadObjectMarking::Side::Left;
            else if (side_str == "rear")
              side = RoadObjectMarking::Side::Rear;
            else if (side_str == "right")
              side = RoadObjectMarking::Side::Right;

            const std::string weight_str =
                marking_node.attribute("weight").as_string("standard");
            RoadObjectMarking::Weight weight =
                RoadObjectMarking::Weight::Standard;
            if (weight_str == "bold")
              weight = RoadObjectMarking::Weight::Bold;

            RoadObjectMarking marking(
                side, weight, marking_node.attribute("width").as_double(0.0),
                marking_node.attribute("color").as_string(""),
                marking_node.attribute("zOffset").as_double(0.0),
                marking_node.attribute("spaceLength").as_double(0.0),
                marking_node.attribute("lineLength").as_double(0.0),
                marking_node.attribute("startOffset").as_double(0.0),
                marking_node.attribute("stopOffset").as_double(0.0));

            for (pugi::xml_node corner_ref_node :
                 marking_node.children("cornerReference")) {
              marking.corner_references.emplace_back(
                  corner_ref_node.attribute("id").as_int(0));
            }
            road_object.markings.push_back(std::move(marking));
          }
        }

        for (pugi::xml_node border_node : object_node.children("border")) {
          const std::string type_str =
              border_node.attribute("type").as_string("concrete");
          RoadObjectBorder::Type type = RoadObjectBorder::Type::Concrete;
          if (type_str == "curb")
            type = RoadObjectBorder::Type::Curb;
          else if (type_str == "paint")
            type = RoadObjectBorder::Type::Paint;

          road_object.borders.emplace_back(
              border_node.attribute("width").as_double(0.0), type,
              border_node.attribute("outlineId").as_int(0),
              border_node.attribute("useForMapping").as_bool(false));
        }

        for (pugi::xml_node skeleton_node : object_node.children("skeleton")) {
          RoadObjectSkeleton skeleton(skeleton_node.attribute("id").as_int(0));
          for (pugi::xml_node polyline_node :
               skeleton_node.children("polyline")) {
            for (pugi::xml_node vertex_node : polyline_node.children()) {
              const std::string node_name = vertex_node.name();
              if (node_name == "vertexLocal") {
                skeleton.vertices.emplace_back(
                    Vec3D{vertex_node.attribute("u").as_double(0.0),
                          vertex_node.attribute("v").as_double(0.0),
                          vertex_node.attribute("z").as_double(0.0)},
                    RoadObjectSkeletonVertex::Type::Local);
              } else if (node_name == "vertexRoad") {
                skeleton.vertices.emplace_back(
                    Vec3D{vertex_node.attribute("s").as_double(0.0),
                          vertex_node.attribute("t").as_double(0.0),
                          vertex_node.attribute("dz").as_double(0.0)},
                    RoadObjectSkeletonVertex::Type::Road);
              }
            }
          }
          road_object.skeletons.push_back(std::move(skeleton));
        }

        road_object.lane_validities =
            extract_lane_validity_records(object_node);
      }

      for (pugi::xml_node ref_node :
           road_node.child("objects").children("objectReference")) {
        road.object_references.emplace_back(
            ref_node.attribute("id").as_string(""),
            ref_node.attribute("s").as_double(0.0),
            ref_node.attribute("t").as_double(0.0),
            ref_node.attribute("zOffset").as_double(0.0),
            ref_node.attribute("validLength").as_double(0.0),
            ref_node.attribute("orientation").as_string(""));
      }
    }
    /* parse signals */
    if (with_road_signals) {
      for (const pugi::xml_node signal_node :
           road_node.child("signals").children("signal")) {
        ...
        odr::check_and_repair(
            ...
            road_id.c_str(), road_signal_id.c_str());

        RoadSignal &road_signal =
            road.id_to_signal
                .insert(
                    {road_signal_id,
                     RoadSignal(
                         road_id, road_signal_id,
        ...
        for (auto [val_name, val_ptr] :
             {std::pair{"s", &(road_signal.s0)},
              ...
              std::pair{"width", &(road_signal.width)}}) {
          odr::check_and_repair(
              (*val_ptr) >= 0, [p = val_ptr]() { *p = 0; },
              "Road #%s Signal #%s: signal::%s %f < 0, set %s=0",
              road_id.c_str(), road_signal_id.c_str(), val_name, *val_ptr,
              val_name);
        }
    ...
    /* parse tunnels - ASAM OpenDRIVE § 13.11 */
    for (pugi::xml_node tunnel_node : road_node.children("tunnel")) {
      const std::string type_str =
          tunnel_node.attribute("type").as_string("standard");
      RoadTunnel::Type type = RoadTunnel::Type::Standard;
      if (type_str == "underpass")
        type = RoadTunnel::Type::Underpass;

      RoadTunnel tunnel(tunnel_node.attribute("id").as_string(""),
                        tunnel_node.attribute("s").as_double(0.0),
                        tunnel_node.attribute("length").as_double(0.0), type,
                        tunnel_node.attribute("lighting").as_double(0.0),
                        tunnel_node.attribute("daylight").as_double(0.0));
      tunnel.lane_validities = extract_lane_validity_records(tunnel_node);
      road.tunnels.push_back(std::move(tunnel));
    }

    /* parse bridges - ASAM OpenDRIVE § 13.12 */
    for (pugi::xml_node bridge_node : road_node.children("bridge")) {
      const std::string type_str =
          bridge_node.attribute("type").as_string("concrete");
      RoadBridge::Type type = RoadBridge::Type::Concrete;
      if (type_str == "steel")
        type = RoadBridge::Type::Steel;
      else if (type_str == "wood")
        type = RoadBridge::Type::Wood;
      else if (type_str == "brick")
        type = RoadBridge::Type::Brick;
      else if (type_str == "extra")
        type = RoadBridge::Type::Extra;
      else if (type_str == "poly")
        type = RoadBridge::Type::Poly;

      RoadBridge bridge(bridge_node.attribute("id").as_string(""),
                        bridge_node.attribute("name").as_string(""),
                        bridge_node.attribute("s").as_double(0.0),
                        bridge_node.attribute("length").as_double(0.0), type);
...
const LaneSection *
OpenDriveMap::get_adjacent_lanesection(const std::string &road_id,
                                       const double &lanesection_s0,
                                       const bool predecessor) const {
  ...
  if (road_link.type == RoadLink::Type::Road &&
      road_link.contact_point != RoadLink::ContactPoint::None) {
    ...
    const LaneSection &adjacent_lanesection =
        (road_link.contact_point == RoadLink::ContactPoint::Start)
...
const Lane *OpenDriveMap::get_adjacent_lane(const Lane &lane,
                                            const bool predecessor) const {
  const LaneSection *adjacent_lanesec = this->get_adjacent_lanesection(
      lane.key.road_id, lane.key.lanesection_s0, predecessor);
  if (!adjacent_lanesec)
    return nullptr;

  const std::optional<int> adjacent_lane_id =
      predecessor ? lane.predecessor : lane.successor;
  if (adjacent_lane_id) {
    const auto adjacent_lane_iter =
        adjacent_lanesec->id_to_lane.find(adjacent_lane_id.value());
      ...
      for (const auto &[_, lane] : lanesection.id_to_lane) {
        ...
        if (predecessor_lane) {
          ...
          routing_graph.add_edge(
              RoutingGraphEdge(predecessor_lane->key, lane.key, lane_length));
        }

        const Lane *successor_lane =
            this->get_adjacent_lane(lane, !lane_follows_road_direction);
        if (successor_lane) {
          const double lane_length =
              road.get_lanesection_length(lane.key.lanesection_s0);
          routing_graph.add_edge(
              RoutingGraphEdge(lane.key, successor_lane->key, lane_length));
        }
      }
    }
  }
  ...
  for (const auto &[_, junction] : id_to_junction) {
    for (const auto &[_, conn] : junction.id_to_connection) {
      ...
      if (incoming_road_iter == id_to_road.end() ||
          connecting_road_iter == id_to_road.end())
        continue;
      ...
      const LaneSection &incoming_lanesec =
          (incoming_road.successor.type == RoadLink::Type::Junction &&
           incoming_road.successor.id == junction.id)
      ...
      const LaneSection &connecting_lanesec =
          (conn.contact_point == JunctionConnection::ContactPoint::Start)
      ...
      for (const JunctionLaneLink &lane_link : conn.lane_links) {
        ...
        const LaneKey from(incoming_road.id, incoming_lanesec.s0,
                           from_lane_iter->second.id);
        const LaneKey to(connecting_road.id, connecting_lanesec.s0,
                         to_lane_iter->second.id);
        const double lane_length =
