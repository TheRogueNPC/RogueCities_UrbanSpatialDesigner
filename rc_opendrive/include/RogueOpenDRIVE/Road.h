...
struct RoadLink {
  enum class ContactPoint { None, Start, End };

  enum class Type { None, Road, Junction };
...
public:
  ...
  Vec3D get_xyz(const double s, const double t, const double h,
                Vec3D *e_s = nullptr, Vec3D *e_t = nullptr,
                Vec3D *e_h = nullptr) const;
  Vec3D get_surface_pt(double s, const double t, Vec3D *vn = nullptr) const;

  Line3D get_lane_border_line(const Lane &lane, const double s_start,
                              const double s_end, const double eps,
                              const bool outer = true) const;
  Line3D get_lane_border_line(const Lane &lane, const double eps,
                              const bool outer = true) const;

  Mesh3D get_lane_mesh(const Lane &lane, const double s_start,
                       const double s_end, const double eps,
                       std::vector<uint32_t> *outline_indices = nullptr) const;
  Mesh3D get_lane_mesh(const Lane &lane, const double eps,
                       std::vector<uint32_t> *outline_indices = nullptr) const;
  ...
  std::set<double>
  approximate_lane_border_linear(const Lane &lane, const double s_start,
                                 const double s_end, const double eps,
                                 const bool outer = true) const;
  std::set<double>
  approximate_lane_border_linear(const Lane &lane, const double eps,
                                 const bool outer = true) const;
