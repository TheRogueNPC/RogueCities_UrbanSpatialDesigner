...
enum class CoordinateSystem { LocalMetric, GlobalProjection };

class OpenDriveMap {
public:
  OpenDriveMap() = default;
  OpenDriveMap(const std::string &xodr_file, const bool center_map = false,
  ...
  const LaneSection *get_adjacent_lanesection(const std::string &road_id,
  ...
  double x_offs = 0;
  double y_offs = 0;
  CoordinateSystem coordinate_system = CoordinateSystem::LocalMetric;
